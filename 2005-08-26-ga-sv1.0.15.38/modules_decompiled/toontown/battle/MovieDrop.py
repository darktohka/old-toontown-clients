...
def __getSoundTrack(level, hitSuit, node = None):
    ...
    if soundEffect:
        ...
        soundTrack.append(SoundInterval(buttonSound, duration = 0.67000000000000004, node = node))
...
def __doSuitDrops(dropTargetPairs, npcs, npcDrops):
    ...
    for dropTargetPair in dropTargetPairs:
        ...
        track = __dropObject(drop, delay, objName, level, alreadyDodged, alreadyTeased, npcs, target, npcDrops)
        if track:
