...
class PetAvatarPanel(AvatarPanel.AvatarPanel):
    ...
    def __init__(self, avatar):
        ...
        self.feedButton = DirectButton(parent = self.frame, image = (gui.find('**/ButtonFeedUp'), gui.find('**/ButtonFeedDown'), gui.find('**/ButtonFeedRollover'), gui.find('**/ButtonFeedUp')), geom = gui.find('**/PetControlFeedIcon'), geom3_color = disabledImageColor, image3_color = disabledImageColor, relief = None, text = TTLocalizer.PetPanelFeed, text_scale = 0.5, text0_fg = text0Color, text1_fg = text1Color, text2_fg = text2Color, text3_fg = text3Color, text_pos = (-0.5, 2.7999999999999998), text_align = TextNode.ALeft, command = self._PetAvatarPanel__handleFeed)
        if not (self.petIsLocal):
            self.feedButton['state'] = DISABLED
        else:
            self.feedButton['state'] = self.feedButtonState()
        self.callButton = DirectButton(parent = self.frame, image = (gui.find('**/ButtonGoToUp'), gui.find('**/ButtonGoToDown'), gui.find('**/ButtonGoToRollover'), gui.find('**/ButtonGoToUp')), geom = gui.find('**/PetControlGoToIcon'), geom3_color = disabledImageColor, image3_color = disabledImageColor, relief = None, text = TTLocalizer.PetPanelCall, text0_fg = text0Color, text1_fg = text1Color, text2_fg = text2Color, text3_fg = text3Color, text_scale = 0.5, text_pos = (-0.5, 1.3), text_align = TextNode.ALeft, command = self._PetAvatarPanel__handleCall)
        if not (self.petIsLocal):
    ...
    def _PetAvatarPanel__handleDetail(self):
        self.petDetailPanel = PetDetailPanel.PetDetailPanel(pet = self.avatar, closeCallback = self._PetAvatarPanel__handleDetailDone, parent = self.frame)
        self.detailButton['state'] = DISABLED
