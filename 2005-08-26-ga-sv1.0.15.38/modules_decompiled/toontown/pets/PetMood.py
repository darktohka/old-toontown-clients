...
class PetMood:
    ...
    def __init__(self, pet = None):
        ...
        self.tSadness = calcDrift(PetMood.TSadness, self.pet.traits.traits['sadnessThreshold'], True)
        self.tFatigue = calcDrift(PetMood.TFatigue, self.pet.traits.traits['fatigueThreshold'], True)
        self.tHunger = calcDrift(PetMood.THunger, self.pet.traits.traits['hungerThreshold'])
        self.tConfusion = calcDrift(PetMood.TConfusion, self.pet.traits.traits['confusionThreshold'], True)
        self.tExcitement = calcDrift(PetMood.TExcitement, self.pet.traits.traits['excitementThreshold'])
        self.tSurprise = calcDrift(PetMood.TSurprise, self.pet.traits.traits['surpriseThreshold'], True)
        self.tAffection = calcDrift(PetMood.TAffection, self.pet.traits.traits['affectionThreshold'])
        self.tAngerDec = calcDrift(PetMood.TAngerDec, self.pet.traits.traits['angerThreshold'], True)
