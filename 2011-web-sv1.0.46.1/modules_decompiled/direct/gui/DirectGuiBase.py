...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\direct\src\gui\DirectGuiBase.py
__all__ = [
 'DirectGuiBase', 'DirectGuiWidget']
        ...
        if len(kw) == 0:
            if option is None:
                ...
                for (option, config) in self._optionInfo.items():
                    rtn[option] = (
                     option, config[DGG._OPT_DEFAULT], config[DGG._OPT_VALUE])
...
class DirectGuiWidget(DirectGuiBase, NodePath):
    ...
    def __init__(self, parent=None, **kw):
        ...
        if self['scale']:
            scale = self['scale']
            if isinstance(scale, VBase3) or type(scale) == types.IntType or type(scale) == types.FloatType:
                self.setScale(scale)
            else:
    ...
    def getBounds(self, state=0):
        ...
        self.bounds = [
         self.ll[0] - self['pad'][0], self.ur[0] + self['pad'][0], self.ll[2] - self['pad'][1], self.ur[2] + self['pad'][1]]
        return self.bounds
