...
def _getcode(f):
    ...
    def instance_get(f):
        if hasattr(f, '__call__'):
            ...
            if type(method) == types.MethodType:
                ...
                func = method
            return (
             '%s%s' % (f.__class__.__name__, '__call__'), func)
        else:
    ...
    def class_get(f):
        if hasattr(f, '__init__'):
            return (
             f.__name__, f.__init__.im_func)
        else:
            return (
             f.__name__, lambda : None)

    codedict = {types.UnboundMethodType: method_get, types.MethodType: method_get, types.FunctionType: function_get, types.InstanceType: instance_get, types.ClassType: class_get}
    try:
...
def _getProfileResultFileInfo(filename):
    return (
     profileFilename2file.get(filename, None), profileFilename2marshalData.get(filename, None))
...
packageMap = {'toontown': '$TOONTOWN', 'direct': '$DIRECT', 'otp': '$OTP', 'pirates': '$PIRATES'}
