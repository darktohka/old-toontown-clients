...
# Embedded file name: C:\cygwin\home\toonpub\player\wintools\sdk\python\Python-2.4.1\Lib\pickle.py
__version__ = '$Revision: 1.1.1.1 $'
from types import *
from copy_reg import dispatch_table
from copy_reg import _extension_registry, _inverted_registry, _extension_cache
import marshal, sys, struct, re, warnings
__all__ = [
 'PickleError', 'PicklingError', 'UnpicklingError', 'Pickler', 'Unpickler', 'dump', 'dumps', 'load', 'loads']
format_version = '2.0'
...
class Pickler:
    ...
    def __init__(self, file, protocol=None, bin=None):
        ...
        if bin is not None:
            warnings.warn("The 'bin' argument to Pickler() is deprecated", DeprecationWarning)
            protocol = bin
        if protocol is None:
            protocol = 0
        if protocol < 0:
    ...
    def save_reduce(self, func, args, state=None, listitems=None, dictitems=None, obj=None):
        if not isinstance(args, TupleType):
            if args is None:
                warnings.warn('__basicnew__ special case is deprecated', DeprecationWarning)
            else:
