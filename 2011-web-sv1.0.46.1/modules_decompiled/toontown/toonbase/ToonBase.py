...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\toonbase\ToonBase.py
...
class ToonBase(OTPBase.OTPBase):
    ...
    def __init__(self):
        if not config.GetInt('ignore-user-options', 0):
            ...
            resList = [(640, 480), (800, 600), (1024, 768), (1280, 1024), (1600, 1200)]
    ...
    def getShardPopLimits(self):
        if self.cr.productName == 'JP':
            return (
             config.GetInt('shard-low-pop', ToontownGlobals.LOW_POP_JP), config.GetInt('shard-mid-pop', ToontownGlobals.MID_POP_JP), config.GetInt('shard-high-pop', ToontownGlobals.HIGH_POP_JP))
        elif self.cr.productName in ['BR', 'FR']:
            return (
             config.GetInt('shard-low-pop', ToontownGlobals.LOW_POP_INTL), config.GetInt('shard-mid-pop', ToontownGlobals.MID_POP_INTL), config.GetInt('shard-high-pop', ToontownGlobals.HIGH_POP_INTL))
        else:
