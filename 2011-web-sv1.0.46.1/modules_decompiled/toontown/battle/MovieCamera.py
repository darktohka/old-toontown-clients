...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\battle\MovieCamera.py
...
def chooseSoundCloseShot(sounds, targets, openDuration, openName, attackDuration):
    ...
    if numSuits == 1:
        av = targets[0]['suit']
        shotChoices = [
         avatarCloseUpThrowShot, avatarCloseUpThreeQuarterLeftShot, allGroupLowShot, suitGroupThreeQuarterLeftBehindShot]
...
def chooseThrowOpenShot(throws, suitThrowsDict, attackDuration):
    ...
    if numThrows == 1:
        av = throws[0]['toon']
        shotChoices = [
         avatarCloseUpThrowShot, avatarCloseUpThreeQuarterRightShot, avatarBehindShot, allGroupLowShot, suitGroupThreeQuarterLeftBehindShot]
...
def chooseSuitCloseShot(attack, openDuration, openName, attackDuration):
    ...
    if groupStatus == ATK_TGT_SINGLE:
        ...
        shotChoices = [
         avatarCloseUpThreeQuarterRightShot, suitGroupThreeQuarterLeftBehindShot]
...
def chooseSOSShot(av, duration):
    shotChoices = [
     avatarCloseUpThreeQuarterRightShot, avatarBehindShot, avatarBehindHighShot, suitGroupThreeQuarterLeftBehindShot]
    track = apply(random.choice(shotChoices), [av, duration])
    return track
...
def chooseRewardShot(av, duration, allowGroupShot=1):
    ...
    def chooseRewardShotNow(av):
        if av.playingAnim == 'victory' or not allowGroupShot:
            shotChoices = [(0, 8, av.getHeight() * 0.66, 179, 15, 0), (5.2, 5.45, av.getHeight() * 0.66, 131.5, 3.6, 0)]
...
def randomCameraSelection(suit, attack, attackDuration, openShotDuration):
    shotChoices = [
     avatarCloseUpThrowShot, avatarCloseUpThreeQuarterLeftShot, allGroupLowShot, suitGroupLowLeftShot, avatarBehindHighShot]
    if openShotDuration > attackDuration:
        openShotDuration = attackDuration
    closeShotDuration = attackDuration - openShotDuration
...
def chooseFireCloseShot(throws, suitThrowsDict, openDuration, openName, attackDuration):
    ...
    if numSuits == 1:
        av = base.cr.doId2do[suitThrowsDict.keys()[0]]
        shotChoices = [
         avatarCloseUpFireShot, avatarCloseUpThreeQuarterLeftFireShot, allGroupLowShot, suitGroupThreeQuarterLeftBehindShot]
    elif numSuits >= 2 and numSuits <= 4 or numSuits == 0:
        shotChoices = [allGroupLowShot, suitGroupThreeQuarterLeftBehindShot]
    else:
        notify.error('Bad number of suits: %s' % numSuits)
