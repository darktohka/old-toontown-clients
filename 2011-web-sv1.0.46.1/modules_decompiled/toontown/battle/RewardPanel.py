...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\battle\RewardPanel.py
...
class RewardPanel(DirectFrame):
    ...
    def __init__(self, name):
        ...
        for i in range(len(ToontownBattleGlobals.Tracks)):
            ...
            self.trackBars.append(DirectWaitBar(parent=self.gagExpFrame, relief=DGG.SUNKEN, frameSize=(-1, 1, -0.15, 0.15), borderWidth=(0.02,
                                                                                                                                         0.02), scale=0.25, frameColor=(ToontownBattleGlobals.TrackColors[i][0] * 0.7, ToontownBattleGlobals.TrackColors[i][1] * 0.7, ToontownBattleGlobals.TrackColors[i][2] * 0.7, 1), barColor=(ToontownBattleGlobals.TrackColors[i][0], ToontownBattleGlobals.TrackColors[i][1], ToontownBattleGlobals.TrackColors[i][2], 1), text='0/0', text_scale=0.18, text_fg=(0,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        0,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        0,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        1), text_align=TextNode.ACenter, text_pos=(0, -0.05), pos=(0.4, 0, -0.09 * i)))
    ...
    def resetMeritBarColor(self, dept):
        self.meritBars[dept]['barColor'] = (
         DisguisePage.DeptColors[dept][0] * 0.8, DisguisePage.DeptColors[dept][1] * 0.8, DisguisePage.DeptColors[dept][2] * 0.8, 1)
    ...
    def uberGagInterval(self, toon, track, level):
        ...
        self.itemFrame.hide()
        self.missedItemFrame.hide()
        self.newGagFrame['text'] = TTLocalizer.RewardPanelUberGag % {'gagName': ToontownBattleGlobals.Tracks[track].capitalize(), 'exp': str(ToontownBattleGlobals.UberSkill), 'avName': toon.getName()}
    ...
    def getUberGagIntervalList(self, toon, track, level):
        ...
        finalDelay = 1.5
        (leftString, rightString) = self.getRandomCongratsPair(toon)
        intervalList = [Func(self.uberGagInterval, toon, track, level), Wait(leftCongratsAnticipate), Func(self.congratsLeft.setProp, 'text', leftString), Wait(rightCongratsAnticipate), Func(self.congratsRight.setProp, 'text', rightString), Wait(finalDelay), Func(self.cleanupNewGag)]
        return intervalList
    ...
    def getQuestIntervalList(self, toon, deathList, toonList, origQuestsList, itemList, helpfulToonsList=[]):
        ...
        for i in range(0, len(deathList), 4):
            ...
            cogList.append({'type': cogType, 'level': cogLevel, 'track': cogTrack, 'isSkelecog': isSkelecog, 'isForeman': isForeman, 'isVP': isVP, 'isCFO': isCFO, 'isSupervisor': isSupervisor, 'isVirtual': isVirtual, 'hasRevives': hasRevives, 'activeToons': activeToonIds})
    ...
    def getExpTrack(self, toon, origExp, earnedExp, deathList, origQuestsList, itemList, missedItemList, origMeritList, meritList, partList, toonList, uberEntry, helpfulToonsList):
