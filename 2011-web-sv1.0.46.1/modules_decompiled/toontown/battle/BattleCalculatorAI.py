...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\battle\BattleCalculatorAI.py
...
class BattleCalculatorAI:
    ...
    def __postProcessToonAttacks(self):
        ...
        for currToonAttack in self.toonAtkOrder:
            if currToonAttack != -1:
                ...
                (atkTrack, atkLevel) = self.__getActualTrackLevel(attack)
                if atkTrack != HEAL:
                    if atkTrack != SOS and atkTrack != NO_ATTACK and atkTrack != NPCSOS and atkTrack != PETSOS:
                    ...
                    if self.itemIsCredit(atkTrack, atkLevel) and (atkTrack == TRAP or atkTrack == LURE):
                        pass
                    elif atkTrack == HEAL:
                        if damagesDone != 0:
                            self.__addAttackExp(attack)
    ...
    def __getActualTrackLevelHp(self, toonAttack):
        if toonAttack[TOON_TRACK_COL] == NPCSOS:
            ...
            return (
             toonAttack[TOON_TRACK_COL], toonAttack[TOON_LVL_COL], hp)
        return (
