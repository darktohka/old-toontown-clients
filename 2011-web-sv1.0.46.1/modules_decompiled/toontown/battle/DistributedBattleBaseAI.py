...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\battle\DistributedBattleBaseAI.py
...
class DistributedBattleBaseAI(DistributedObjectAI.DistributedObjectAI, BattleBase):
    ...
    def getMembers(self):
        ...
        return [suits, joiningSuits, pendingSuits, activeSuits, luredSuits, suitTraps, toons, joiningToons, pendingToons, activeToons, runningToons, globalClockDelta.getRealNetworkTime()]
    ...
    def requestAttack(self, track, level, av):
        ...
        if toon.getGameAccess() != ToontownGlobals.AccessFull:
            if track in [HEAL, TRAP, LURE, SOUND, THROW, SQUIRT, DROP]:
            ...
            if toon.inventory.numItem(track, level) == 0:
                ...
                self.toonAttacks[toonId] = getToonAttack(toonId)
                return
            if track == HEAL and (self.runningToons.count(av) == 1 or attackAffectsGroup(track, level) and len(self.activeToons) < 2):
                self.toonAttacks[toonId] = getToonAttack(toonId, track=UN_ATTACK)
                validResponse = 0
            else:
    ...
    def __movieDone(self):
        ...
        for suit in deadSuits:
            ...
            if suit.isDeleted():
                ...
                encounter = {'type': suit.dna.name, 'level': suit.getActualLevel(), 'track': suit.dna.dept, 'isSkelecog': suit.getSkelecog(), 'isForeman': suit.isForeman(), 'isVP': 0, 'isCFO': 0, 'isSupervisor': suit.isSupervisor(), 'isVirtual': suit.isVirtual(), 'hasRevives': suit.getMaxSkeleRevives(), 'activeToons': self.activeToons[:]}
