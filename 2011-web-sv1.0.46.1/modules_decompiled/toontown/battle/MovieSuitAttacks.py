...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\battle\MovieSuitAttacks.py
...
def getThrowTrack(object, target, duration=1.0, parent=render, gravity=-32.144):
    ...
    return Sequence(Func(calcOriginAndVelocity), LerpFunctionInterval(throwPos, fromData=0.0, toData=1.0, duration=duration, extraArgs=[object, duration, target, values, gravity]))
...
def doShred(attack):
    ...
    shredderPosPoints = [
     Point3(0, -0.12, -0.34), VBase3(-90.0, -53.77, -0.0)]
...
def doWriteOff(attack):
    ...
    pencilPosPoints = [
     Point3(-0.47, 1.08, 0.28), VBase3(21.045, 12.702, -176.374)]
...
def doRazzleDazzle(attack):
    ...
    suitTrack = getSuitTrack(attack)
    signPosPoints = [
     Point3(0.0, -0.42, -0.04), VBase3(105.715, 73.977, 65.932)]
    if hitSuit:
        hitPoint = lambda toon=toon: __toonFacePoint(toon)
    else:
