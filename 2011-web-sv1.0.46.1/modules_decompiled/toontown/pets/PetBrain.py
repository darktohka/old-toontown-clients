...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\pets\PetBrain.py
...
class PetBrain(DirectObject.DirectObject, CPetBrain):
    ...
    def _handleActionObserve(self, observe):
        ...
        if action == OA.ATTENDED_START:
            ...
            self.pet.lerpMoods({'boredom': -0.1, 'excitement': 0.05, 'loneliness': -0.05})
            ...
            self.pet.lerpMoods({'affection': 0.35, 'anger': -0.07, 'boredom': -0.5, 'excitement': 0.5, 'fatigue': -0.2, 'hunger': -0.5, 'loneliness': -0.08, 'playfulness': 0.1, 'restlessness': -0.05, 'sadness': -0.2})
            ...
            self.pet.lerpMoods({'affection': 0.45, 'anger': -0.1, 'boredom': -0.8, 'excitement': 0.5, 'fatigue': -0.25, 'loneliness': -0.2, 'playfulness': 0.1, 'restlessness': -0.2, 'sadness': -0.2})
            self.updateLastInteractTime(avId)
            avatar = simbase.air.doId2do.get(avId)
            if avatar is not None:
                avatar.setHatePets(0)
    ...
    def _handlePhraseObserve(self, observe):
        ...
        def _handleGettingFriendlyAttention(avId, self=self):
            self.pet.lerpMoods({'boredom': -0.85, 'restlessness': -0.1, 'playfulness': 0.2, 'loneliness': -0.4, 'sadness': -0.1, 'fatigue': -0.05, 'excitement': 0.05, 'anger': -0.05})
            self.updateLastInteractTime(avId)
        ...
        def _handleCriticism(avId, self=self):
            ...
            if avId == self.pet.ownerId:
                ownerFactor = 1.0
            self.pet.lerpMoods({'affection': -0.4, 'anger': 0.4, 'boredom': -0.3, 'confusion': 0.05, 'fatigue': 0.2, 'playfulness': -0.1, 'sadness': 0.5 * ownerFactor})
        ...
        if phrase in list2dict([OP.COME, OP.FOLLOW_ME, OP.STAY, OP.NEED_LAFF, OP.NEED_GAGS, OP.NEED_JB, OP.HI, OP.SOOTHE, OP.PRAISE, OP.HAPPY, OP.QUESTION, OP.FRIENDLY, OP.LETS_PLAY, OP.DO_TRICK]):
