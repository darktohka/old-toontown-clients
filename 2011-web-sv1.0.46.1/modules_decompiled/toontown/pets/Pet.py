...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\pets\Pet.py
...
import random, types
Component2IconDict = {'boredom': 'Bored', 'restlessness': None, 'playfulness': 'Play', 'loneliness': 'Lonely', 'sadness': 'Sad', 'fatigue': 'Sleepy', 'hunger': 'Hungry', 'confusion': 'Confused', 'excitement': 'Surprised', 'anger': 'Angry', 'surprise': 'Surprised', 'affection': 'Love'}
...
class Pet(Avatar.Avatar):
    ...
    InteractAnims = {Interactions.SCRATCH: ('toPet', 'pet', 'fromPet'), Interactions.BEG: ('toBeg', 'beg', 'fromBeg'), Interactions.EAT: ('eat', 'swallow', 'neutral'), Interactions.NEUTRAL: 'neutral'}
...
def gridPets():
    ...
    for body in range(0, len(BodyTypes)):
        ...
        for color in colors:
            p = Pet()
            p.setDNA([random.choice(range(-1, len(HeadParts))), random.choice(range(-1, len(EarParts))), random.choice(range(-1, len(NoseParts))), random.choice(range(-1, len(TailParts))), body, color, random.choice(range(-1, len(ColorScales))), random.choice(range(0, len(PetEyeColors))), random.choice(range(0, len(PetGenders)))])
