...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\ai\ToontownMagicWordManager.py
...
class ToontownMagicWordManager(MagicWordManager.MagicWordManager):
            ...
            if wordIs('~fanfare'):
                ...
                if wordIs('~undead'):
                    ...
                    if base.wantPets:
                        ...
                        pet = self.cr.doId2do.get(petId)
                        if pet:
                            pet.isLockedDown() or pet.lockPet()
    ...
    def doParty(self, word, av, zoneId):
        ...
        if action == 'plan':
            ...
            if shardId == base.localAvatar.defaultShard:
                shardId = None
            base.cr.playGame.getPlace().requestLeave({'loader': 'safeZoneLoader', 'where': 'party', 'how': 'teleportIn', 'hoodId': hoodId, 'zoneId': -1, 'shardId': shardId, 'avId': -1})
    ...
    def requestTeleport(self, loaderId, whereId, hoodId, zoneId, avId):
        ...
        if avId == 0:
            avId = -1
        place.fsm.forceTransition('teleportOut', [{'loader': loaderId, 'where': whereId, 'how': 'teleportIn', 'hoodId': hoodId, 'zoneId': zoneId, 'shardId': None, 'avId': avId}])
        return
