...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\minigame\DistributedPhotoGame.py
...
class DistributedPhotoGame(DistributedMinigame, PhotoGameBase.PhotoGameBase):
    ...
    def load(self):
        ...
        self.filmPanel = DirectLabel(parent=hidden, relief=None, pos=(1.16, 0.0, 0.45), scale=0.65, text=str(self.filmCount), text_scale=0.2, text_fg=(0.95,
                                                                                                                                                       0.95,
                                                                                                                                                       0,
                                                                                                                                                       1), text_pos=(0.08, -0.15), text_font=ToontownGlobals.getSignFont(), image=self.filmImage, image_scale=Point3(1.0, 0.0, 0.85))
    ...
    def __testCollisions(self):
        ...
        for key in hitDict:
            ...
            if not centerDict.has_key(superParent):
                ...
                centerDict[superParent] = (
                 onCenter + centerDict[superParent][0], overB + centerDict[superParent][1], overT + centerDict[superParent][2], overR + centerDict[superParent][3], overL + centerDict[superParent][4])
    ...
    def determinePhotoContent(self, subject):
        if self.getSubjectTrackState(subject):
            return [
             subject, self.getSubjectTrackState(subject)[2]]
        else:
            return
        return
    ...
    def makeAssignmentFrame(self):
        ...
        frame = DirectFrame(parent=hidden, image=photoImage, image_color=(1, 1, 1,
                                                                          1), image_scale=Point3(1.6 * assignmentScale, 0.0, 1.75 * assignmentScale), frameSize=(-size, size, -size, size), text='HC Score', textMayChange=1, text_wordwrap=9, text_pos=Point3(0.0, -0.135, 0.0), text_scale=0.045, relief=None)
        return frame
    ...
    def makeScoreFrame(self):
        ...
        frame = DirectFrame(parent=hidden, image=scoreImage, image_color=(1, 1, 1,
                                                                          1), image_scale=Point3(0.64, 0.0, 0.64), frameSize=(-size, size, -size, size), text='Score Frame', textMayChange=1, text_wordwrap=9, text_pos=Point3(0.0, 0.0, 0.0), text_scale=0.05, relief=None)
        return frame
