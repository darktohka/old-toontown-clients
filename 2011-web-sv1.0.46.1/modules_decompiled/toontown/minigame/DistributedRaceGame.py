...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\minigame\DistributedRaceGame.py
...
class DistributedRaceGame(DistributedMinigame):
    ...
    def __init__(self, cr):
        ...
        self.modelCount = 8
        self.cameraTopView = (
         -22.78, -41.65, 31.53, -51.55, -42.68, -2.96)
        self.timer = None
    ...
    def load(self):
        ...
        for i in range(1, 5):
            ...
            diceButton = DirectButton(image=(button, button_down, button_ro, None), relief=None, pos=(-0.9 + (i - 1) * 0.2, 0.0, -0.85), scale=0.25, command=self.handleInputChoice, extraArgs=[i])
            diceButton.hide()
    ...
    def enterMoveAvatars(self, choiceList, positionList, rewardList):
        ...
        for i in range(0, len(choiceList) / self.numPlayers):
            ...
            if longestLerpTime > 0.0:
                ...
                if i == 0:
                    tasks += [Task.pause(0.75), mat, Task.pause(0.75), Task(self.hideNumbers), Task.pause(longestLerpTime - 0.5)]
                else:
                    mat.chance = 1
