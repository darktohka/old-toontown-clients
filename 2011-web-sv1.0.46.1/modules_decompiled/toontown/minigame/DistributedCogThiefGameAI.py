...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\minigame\DistributedCogThiefGameAI.py
import random
...
class DistributedCogThiefGameAI(DistributedMinigameAI.DistributedMinigameAI):
    ...
    def initCogInfo(self):
        for cogIndex in xrange(self.getNumCogs()):
            self.cogInfo[cogIndex] = {'pos': Point3(CogThiefGameGlobals.CogStartingPositions[cogIndex]), 'goal': CTGG.NoGoal, 'goalId': CTGG.InvalidGoalId, 'barrel': CTGG.NoBarrelCarried}
    ...
    def d_makeCogCarryBarrel(self, timestamp, clientStamp, cogIndex, barrelIndex):
        ...
        self.sendUpdate('makeCogCarryBarrel', [timestamp, clientStamp, cogIndex, barrelIndex, pos[0], pos[1], pos[2]])
    ...
    def d_makeCogDropBarrel(self, timestamp, clientStamp, cogIndex, barrelIndex, barrelPos):
        pos = barrelPos
        gameTime = self.getCurrentGameTime()
        self.sendUpdate('makeCogDropBarrel', [timestamp, clientStamp, cogIndex, barrelIndex, pos[0], pos[1], pos[2]])
    ...
    def runAway(self, clientStamp, cogIndex, cogPos, barrelIndex, returnPosIndex):
        ...
        timestamp = globalClockDelta.localToNetworkTime(globalClock.getFrameTime(), bits=32)
        gameTime = self.getCurrentGameTime()
        self.sendUpdate('updateSuitGoal', [timestamp, clientStamp, cogIndex, goalType, goalId, cogPos[0], cogPos[1], cogPos[2]])
    ...
    def findClosestUnassignedBarrel(self, suitNum):
        ...
        for key in self.barrelInfo:
            ...
            if info['carriedBy'] == CTGG.BarrelOnGround:
                if not info['stolen']:
                    self.isCogGoingForBarrel(key) or possibleBarrels.append(key)
