...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\minigame\DistributedIceGame.py
import math
...
class DistributedIceGame(DistributedMinigame.DistributedMinigame, DistributedIceWorld.DistributedIceWorld):
    ...
    def __aimTask(self, task):
        ...
        if self.arrowKeys.upPressed():
            if not self.arrowKeys.downPressed():
            ...
            if self.arrowKeys.leftPressed():
                ...
                if arrowRotating:
                    ...
                    self.arrowRotateSound.stop()
                if arrowUp:
                    self.arrowUpSound.status() == self.arrowUpSound.PLAYING or base.playSfx(self.arrowUpSound, looping=False)
            else:
                self.arrowUpSound.stop()
            if arrowDown:
                self.arrowDownSound.status() == self.arrowDownSound.PLAYING or base.playSfx(self.arrowDownSound, looping=False)
        else:
            self.arrowDownSound.stop()
        return task.cont
    ...
    def updateInfoLabel(self):
        self.infoLabel['text'] = TTLocalizer.IceGameInfo % {'curMatch': self.curMatch + 1, 'numMatch': IceGameGlobals.NumMatches, 'curRound': self.curRound + 1, 'numRound': IceGameGlobals.NumRounds}
