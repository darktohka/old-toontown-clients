...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\toon\DistributedToonAI.py
...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    def handleLogicalZoneChange(self, newZoneId, oldZoneId):
        ...
        if not simbase.air.cogSuitMessageSent:
            if self.cogIndex != -1 and not ToontownAccessAI.canWearSuit(self.doId, newZoneId):
                ...
                stack = StackTrace()
                self.notify.warning('handleLogicalZoneChange: %s Stacktrace: %s' % (self.cogIndex, stack.compact()))
            ...
            if zoneId == ToontownGlobals.GoofySpeedway:
                self.hasTeleportAccess(ToontownGlobals.GoofySpeedway) or self.addTeleportAccess(zoneId)
    ...
    def restockAllNPCFriends(self):
        desiredNpcFriends = [
         2001, 2011, 3112, 4119, 1116, 3137, 3135]
    ...
    def setCogIndex(self, index):
        if not simbase.air.cogSuitMessageSent and index != -1 and not ToontownAccessAI.canWearSuit(self.doId, self.zoneId):
            from direct.showbase.PythonUtil import StackTrace
            stack = StackTrace()
            self.notify.warning('setCogIndex: %s Stacktrace: %s' % (index, stack.compact()))
            if simbase.config.GetBool('want-ban-wrong-suit-place', False):
    ...
    def setCheesyEffect(self, effect, hoodId, expireTime):
        if simbase.air.holidayManager and ToontownGlobals.WINTER_CAROLING not in simbase.air.holidayManager.currentHolidays and effect == ToontownGlobals.CESnowMan:
    ...
    def findClosestDoor(self):
        ...
        if not bm:
            return
        zones = [
         zoneId, zoneId - 1, zoneId + 1, zoneId - 2, zoneId + 2]
        for zone in zones:
