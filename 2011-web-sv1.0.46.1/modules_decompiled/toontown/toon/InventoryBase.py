...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\toon\InventoryBase.py
...
class InventoryBase(DirectObject.DirectObject):
    ...
    def addItems(self, track, level, amount):
        ...
        if hasattr(self.toon, 'experience'):
            if hasattr(self.toon.experience, 'getExpLevel'):
                if self.toon.experience.getExpLevel(track) >= level and self.toon.hasTrackAccess(track):
                    if self.numItem(track, level) <= max - amount and (self.totalProps + amount <= self.toon.getMaxCarry() or level > LAST_REGULAR_GAG_LEVEL):
    ...
    def getTrackAndLevel(self, propName):
        for track in range(0, len(Tracks)):
            if AvProps[track].count(propName):
                return (
                 tracks, AvProps[track].index(propName))
