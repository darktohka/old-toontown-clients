...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\toon\LaffMeter.py
...
class LaffMeter(DirectFrame):
    ...
    def load(self):
        ...
        if self.isToon:
            ...
            self.teeth = [
             self.tooth6, self.tooth5, self.tooth4, self.tooth3, self.tooth2, self.tooth1]
            self.fractions = [
             0.0, 0.166666, 0.333333, 0.5, 0.666666, 0.833333]
        gui.removeNode()
        return
        ...
        if self.isToon:
            if self.hp != None:
                ...
                if self.hp < 1:
                    ...
                    self.adjustTeeth()
                self.adjustText()
                quietly or self.animatedEffect(delta)
        return
