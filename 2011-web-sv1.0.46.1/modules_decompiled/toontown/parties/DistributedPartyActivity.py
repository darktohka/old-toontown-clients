...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\parties\DistributedPartyActivity.py
...
class DistributedPartyActivity(DistributedObject.DistributedObject):
    ...
    def _enableHandleToonDisabled(self, toonId):
        toon = self.getAvatar(toonId)
        if toon is not None:
            self.acceptOnce(toon.uniqueName('disable'), self.handleToonDisabled, [
             toonId])
        else:
