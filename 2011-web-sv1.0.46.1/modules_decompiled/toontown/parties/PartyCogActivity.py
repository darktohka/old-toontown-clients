...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\parties\PartyCogActivity.py
...
class PartyCogActivity(DirectObject):
    ...
    def load(self):
        ...
        self.teamCamPosLocators = (
         self.arena.find('**/team0CamPos_locator'), self.arena.find('**/team1CamPos_locator'))
        self.teamCamAimLocators = (
         self.arena.find('**/team0CamAim_locator'), self.arena.find('**/team1CamAim_locator'))
        ...
        self.teamLocators = (
         leftTeamLocator, rightTeamLocator)
        ...
        self._arenaFlagGroups = (
         self.arena.find('**/flagsL_grp'), self.arena.find('**/flagsR_grp'))
    ...
    def _initArenaDoors(self):
        ...
        arenaDoorLocators = (
         self.arena.find('**/doorL_locator'), self.arena.find('**/doorR_locator'))
    ...
    def _playArenaDoorIval(self, team, opening=True):
        ...
        if ival is not None:
            if ival.isPlaying():
                ival.pause()
            pos = opening or self._doorStartPos[team]
        else:
            pos = (
             self._doorStartPos[team] + Point3(0, 0, -7.0),)
    ...
    def getTossPieInterval(self, toon, x, y, z, h, p, r, power, beginFlyIval=Sequence()):
        ...
        toss = Track((0,
         Sequence(Func(toon.setPosHpr, x, y, z, h, p, r), Func(pie.reparentTo, toon.rightHand), Func(pie.setPosHpr, 0, 0, 0, 0, 0, 0), animPie, Parallel(ActorInterval(toon, 'throw', startFrame=48, playRate=1.5, partName='torso'), animPie), Func(__safeSetAnimState, toon, 'Happy'))), (
         16.0 / 24.0, Func(pie.detachNode)))
        fly = Track((14.0 / 24.0, SoundInterval(sound, node=toon, cutOff=PartyGlobals.PARTY_COG_CUTOFF)), (
         16.0 / 24.0,
         Sequence(Func(flyPie.reparentTo, render), Func(flyPie.setPosHpr, toon, 0.52, 0.97, 2.24, 0, -45, 0), beginFlyIval, ProjectileInterval(flyPie, startVel=getVelocity, duration=6), Func(flyPie.detachNode))))
        return (
         toss, fly, flyPie)
