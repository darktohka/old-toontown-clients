...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\parties\PartyCog.py
import math
...
class PartyCog(FSM):
    ...
    def load(self):
        ...
        self.actor = Actor(path + 'actor', {'idle': path + 'idle_anim', 'down': path + 'down_anim', 'up': path + 'up_anim', 'bodyHitBack': path + 'bodyHitBack_anim', 'bodyHitFront': path + 'bodyHitFront_anim', 'headHitBack': path + 'headHitBack_anim', 'headHitFront': path + 'headHitFront_anim'})
    ...
    def __showSplat(self, position, direction, hot=False):
        if self.kaboomTrack is not None:
            if self.kaboomTrack.isPlaying():
                self.kaboomTrack.finish()
            self.clearHitInterval()
            splatName = 'splat-creampie'
            self.splat = globalPropPool.getProp(splatName)
            ...
            direction == 1.0 or self.splat.setColorScale(PartyGlobals.CogActivitySplatColors[0])
