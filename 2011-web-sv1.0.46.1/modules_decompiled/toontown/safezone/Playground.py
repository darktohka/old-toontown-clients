...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\safezone\Playground.py
...
class Playground(Place.Place):
    ...
    def __init__(self, loader, parentFSM, doneEvent):
        ...
        self.fsm = ClassicFSM.ClassicFSM('Playground', [
         ...
         State.State('walk', self.enterWalk, self.exitWalk, [
          'drive', 'sit', 'stickerBook', 'TFA', 'DFA', 'trialerFA', 'trolley', 'final', 'doorOut', 'options', 'quest', 'purchase', 'stopped', 'fishing']),
         State.State('stickerBook', self.enterStickerBook, self.exitStickerBook, [
          'walk', 'DFA', 'TFA', 'trolley', 'final', 'doorOut', 'quest', 'purchase', 'stopped', 'fishing', 'trialerFA']),
    ...
    def handleTrolleyDone(self, doneStatus):
        ...
        if mode == 'reject':
            ...
            self.doneStatus = {'loader': 'minigame', 'where': 'minigame', 'hoodId': self.loader.hood.id, 'zoneId': doneStatus['zoneId'], 'shardId': None, 'minigameId': doneStatus['minigameId']}
    ...
    def debugStartMinigame(self, zoneId, minigameId):
        self.doneStatus = {'loader': 'minigame', 'where': 'minigame', 'hoodId': self.loader.hood.id, 'zoneId': zoneId, 'shardId': None, 'minigameId': minigameId}
        messenger.send(self.doneEvent)
        return
    ...
    def enterTeleportIn(self, requestStatus):
        ...
        if self.dialog:
            ...
            if (Quests.avatarHasTrolleyQuest(base.localAvatar) or Quests.avatarHasFirstCogQuest(base.localAvatar) or Quests.avatarHasFriendQuest(base.localAvatar) or Quests.avatarHasPhoneQuest(base.localAvatar) and Quests.avatarHasCompletedPhoneQuest(base.localAvatar)) and self.loader.hood.id == ToontownGlobals.ToontownCentral:
                requestStatus['nextState'] = 'popup'
                imageModel = loader.loadModel('phase_4/models/gui/tfa_images')
                if base.localAvatar.quests[0][0] == Quests.TROLLEY_QUEST_ID:
                    (x, y, z, h, p, r) = Quests.avatarHasCompletedTrolleyQuest(base.localAvatar) or base.cr.hoodMgr.getDropPoint(base.cr.hoodMgr.ToontownCentralInitialDropPoints)
