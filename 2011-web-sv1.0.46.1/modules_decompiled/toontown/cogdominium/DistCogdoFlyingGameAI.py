...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\cogdominium\DistCogdoFlyingGameAI.py
import random
...
class DistCogdoFlyingGameAI(DistCogdoGameAI):
    ...
    def d_broadcastDebuffPowerup(self, senderId, pickupType):
        self.sendUpdate('debuffPowerup', [senderId, pickupType, self.getCurrentNetworkTime()])
    ...
    def pickedUpInvulPowerup(self, senderId):
        ...
        taskMgr.doMethodLater(Globals.Gameplay.InvulBuffTime, self.b_broadcastDebuffPowerup, self.uniqueName(DistCogdoFlyingGameAI.InvulBuffRemoveTaskName % senderId), extraArgs=[senderId, Globals.Level.GatherableTypes.InvulPowerup])
    ...
    def b_toonSetAsEagleTarget(self, toonId, eagleId, networkTime):
        if eagleId not in self.eagleId2targetIds:
            ...
            if not taskMgr.hasTaskNamed(DistCogdoFlyingGameAI.EagleExitCooldownTaskName % eagleId):
                taskMgr.doMethodLater(self.getLegalEagleAttackRoundTime(), self.d_broadcastEagleExitCooldown, self.uniqueName(DistCogdoFlyingGameAI.EagleExitCooldownTaskName % eagleId), extraArgs=[eagleId])
    ...
    def d_broadcastEagleExitCooldown(self, eagleId):
        ...
        if eagleId in self.eagleId2targetIds:
            ...
            if eagleId in self.eagleId2targetIds:
                taskMgr.doMethodLater(self.getLegalEagleAttackRoundTime(fromCooldown=True), self.d_broadcastEagleExitCooldown, self.uniqueName(DistCogdoFlyingGameAI.EagleExitCooldownTaskName % eagleId), extraArgs=[eagleId])
