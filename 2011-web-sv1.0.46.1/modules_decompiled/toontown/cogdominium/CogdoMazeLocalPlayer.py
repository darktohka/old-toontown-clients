...
# Embedded file name: C:\Cygwin\home\toonpub\player_1_0_46_qa\toontown\src\cogdominium\CogdoMazeLocalPlayer.py
...
class CogdoMazeLocalPlayer(CogdoMazePlayer):
    ...
    def __init__(self, id, toon, game, guiMgr):
        ...
        self.exited = False
        self.hints = {'find': False, 'throw': False, 'squashed': False, 'boss': False, 'minion': False}
    ...
    def handleGameStart(self):
        ...
        self.notify.info('toonId:%d laff:%d/%d  %d player(s) started maze game' % (self.toon.doId, self.toon.hp, self.toon.maxHp, len(self.game.players)))
    ...
    def handleToonEntersDoor(self, toonId, door):
        ...
        self.notify.info('toonId:%d laff:%d/%d  %d player(s) succeeded in maze game. Going to the executive suit building.' % (toonId, self.toon.hp, self.toon.maxHp, len(self.game.players)))
    ...
    def lostMemos(self):
        ...
        self._guiMgr.setPickupCount(self.pickupCount)
        self.notify.info('toonId:%d laff:%d/%d  %d player(s) failed in maze game' % (self.toon.doId, self.toon.hp, self.toon.maxHp, len(self.game.players)))