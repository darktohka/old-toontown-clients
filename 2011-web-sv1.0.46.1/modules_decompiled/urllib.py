...
# Embedded file name: C:\cygwin\home\toonpub\player\wintools\sdk\python\Python-2.4.1\Lib\urllib.py
    ...
    def proxy_bypass(host):
        ...
        while i < len(proxyOverride):
            if proxyOverride[i] == '<local>':
                proxyOverride[i:(i + 1)] = [
                 'localhost', '127.0.0.1', socket.gethostname(), socket.gethostbyname(socket.gethostname())]
            i += 1
...
def test(args=[]):
    if not args:
        args = [
         '/etc/passwd', 'file:/etc/passwd', 'file://localhost/etc/passwd', 'ftp://ftp.python.org/pub/python/README', 'http://www.python.org/index.html']
