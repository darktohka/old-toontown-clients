...
from PandaModules import VBase4
...
class DisguisePage(ShtikerPage.ShtikerPage):
    ...
    def updateMeritBar(self, dept):
        ...
        totalMerits = CogDisguiseGlobals.getTotalMerits(toonbase.localToon, dept)
        progress = min(merits / float(totalMerits), 1)
        self.updateMeritDial(progress)
        if toonbase.localToon.readyForPromotion(dept):
            self.cogMeritRatio['text'] = Localizer.DisguisePageMeritFull
            self.promotionTitle.show()
            self.meritTitle.hide()
        else:
            self.cogMeritRatio['text'] = '%d/%d' % (merits, totalMerits)
            self.promotionTitle.hide()
            self.meritTitle.show()
    ...
    def doTab(self, index):
        ...
        self.deptLabel['text'] = (AvatarDNA.suitDeptFullnames[AvatarDNA.suitDepts[index]],)
        cogIndex = toonbase.localToon.cogTypes[index] + AvatarDNA.suitsPerDept * index
        cog = AvatarDNA.suitHeadTypes[cogIndex]
        self.cogName['text'] = SuitBattleGlobals.SuitAttributes[cog]['name']
        cogLevel = toonbase.localToon.cogLevels[index]
        self.cogLevel['text'] = Localizer.DisguisePageCogLevel % str(cogLevel + 1)
        numParts = toonbase.localToon.cogParts[index]
