...
class DistributedDoor(DistributedObject.DistributedObject):
    ...
    def _DistributedDoor__faRejectEnter(self, message):
        ...
        toonbase.localToon.b_setAnimState('neutral')
    ...
    def exitCompleted(self):
        toonbase.localToon.setAnimState('neutral')
        self.cr.playGame.getPlace().setState('walk')
    ...
    def exitDoorEnterClosing(self, ts):
        ...
        if not leftDoor.isEmpty():
            ...
            self.finishDoorExitTrack()
            self.doorExitTrack = Sequence(LerpHprInterval(nodePath = leftDoor, duration = 1.0, hpr = VBase3(0, 0, 0), startHpr = VBase3(h, 0, 0), other = otherNP, blendType = 'easeInOut'), Func(doorFrameHoleLeft.hide), Func(leftDoor.hide), SoundInterval(self.closeSfx, node = leftDoor), name = trackName)
