    ...
    def findNumberedNodes(baseString, model, caseInsens = 1):
        ...
        for potentialNode in potentialNodes:
            ...
            if num == LevelConstants.UberZoneEntId:
                print 'warning: cannot use UberZone zoneNum (%s). ignoring %s' % (LevelConstants.UberZoneEntId, potentialNode)
                continue
            
            if num < LevelConstants.MinZoneNum or num > LevelConstants.MaxZoneNum:
                print 'warning: zone %s is out of range. ignoring %s' % (num, potentialNode)
                continue
            
            if num in num2node:
                print 'warning: zone %s already assigned to %s. ignoring %s' % (num, num2node[num], potentialNode)
                continue
