...
class SCFactoryMenu(SCMenu):
    
    def __init__(self):
        SCMenu.__init__(self)
        self.meetMenu = SCMenu()
        for msgIndex in Localizer.SCFactoryMeetMenuIndexes:
            term = SCStaticTextTerminal(msgIndex)
            self.meetMenu.append(term)
        
        self.meetMenuHolder = SCMenuHolder.SCMenuHolder(Localizer.SCMenuFactoryMeet, self.meetMenu)
        self[0:0] = [
            self.meetMenuHolder]
        self.accept('factoryZoneChanged', self._SCFactoryMenu__zoneChanged)
        self._SCFactoryMenu__zoneChanged()
    ...
    def _SCFactoryMenu__zoneChanged(self, zoneId = 0):
        del self[0]
        self.clearMenu()
