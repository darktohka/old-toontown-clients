...
class DistributedFireworkShow(DistributedObject.DistributedObject):
    ...
    def getFireworkShowIval(self, holidayId, index, startT):
        ...
        if holidayId == JULY4_FIREWORKS:
            startMessage = Localizer.FireworksJuly4Beginning
            endMessage = Localizer.FireworksJuly4Ending
            musicFile = 'phase_4/audio/bgm/firework_music.mid'
        elif holidayId == NEWYEARS_FIREWORKS:
            startMessage = Localizer.FireworksNewYearsEveBeginning
            endMessage = Localizer.FireworksNewYearsEveEnding
        ...
        dark = 0.5
        skyDark = 0.0
        preShow = Sequence(Func(toonbase.localToon.setSystemMessage, 0, startMessage), Func(checkDDFog), Parallel(LerpColorScaleInterval(self.getGeom(), 2, Vec4(dark, dark, dark, 1)), LerpColorScaleInterval(self.getSky(), 2, Vec4(skyDark, skyDark, skyDark, 1))), Func(toonbase.localToon.setSystemMessage, 0, Localizer.FireworksInstructions), Wait(2), Func(self.getLoader().music.stop), Func(base.playMusic, showMusic, 0, 1, 0.80000000000000004, max(0, startT - 4)))
        mainShow = Sequence()
        currentT = 4.0
        for effect in show:
        ...
        postShow = Sequence(Wait(4), Func(restoreDDFog), Parallel(LerpColorScaleInterval(self.getGeom(), 2, Vec4(1, 1, 1, 1)), LerpColorScaleInterval(self.getSky(), 2, Vec4(1, 1, 1, 1))), Func(self.getGeom().clearColorScale), Func(self.getSky().clearColorScale), Func(showMusic.stop), Func(toonbase.localToon.setSystemMessage, 0, endMessage), Wait(0.5), Func(base.playMusic, self.getLoader().music, 1, 1, 0.80000000000000004))
