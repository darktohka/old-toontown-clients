...
class RewardPanel(DirectFrame):
    ...
    def __init__(self, name):
        ...
        self.cogPartFrame = DirectFrame(parent = self, relief = None, text = Localizer.RewardPanelCogPart, text_pos = (0, 0.20000000000000001), text_scale = 0.080000000000000002)
        ...
        self.questFrame = DirectFrame(parent = self, relief = None, text = Localizer.RewardPanelToonTasks, text_pos = (0, 0.20000000000000001), text_scale = 0.059999999999999998)
        self.questLabelList = []
        for i in range(ToontownGlobals.MaxQuestCarryLimit):
            label = DirectLabel(parent = self.questFrame, relief = None, pos = (-0.84999999999999998, 0, -0.10000000000000001 * i), text = Localizer.RewardPanelQuestLabel % i, text_scale = 0.050000000000000003, text_align = TextNode.ALeft)
            label.hide()
        ...
        self.meritIncLabels = []
        self.meritBars = []
        self.meritBarTopLabel = DirectLabel(parent = self.gagExpFrame, relief = None, pos = (0.82499999999999996, 0, -0.050000000000000003), text = Localizer.RewardPanelMeritBarLabel, text_pos = (0, 0), text_scale = 0.050000000000000003)
        self.meritBarTopLabel.hide()
        for i in range(len(AvatarDNA.suitDepts)):
            deptName = AvatarDNA.suitDeptFullnames[AvatarDNA.suitDepts[i]].upper()
            ...
            self.meritIncLabels.append(DirectLabel(parent = self.gagExpFrame, relief = None, text = '', text_scale = 0.050000000000000003, text_align = TextNode.ALeft, pos = (1.075, 0, -0.089999999999999997 * i - 0.125), text_pos = (0, -0.02)))
            self.meritBars.append(DirectWaitBar(parent = self.gagExpFrame, relief = SUNKEN, frameSize = (-1, 1, -0.14999999999999999, 0.14999999999999999), borderWidth = (0.02, 0.02), scale = 0.25, frameColor = (DisguisePage.DeptColors[i][0] * 0.69999999999999996, DisguisePage.DeptColors[i][1] * 0.69999999999999996, DisguisePage.DeptColors[i][2] * 0.69999999999999996, 1), barColor = (DisguisePage.DeptColors[i][0], DisguisePage.DeptColors[i][1], DisguisePage.DeptColors[i][2], 1), text = '0/0', text_scale = 0.17999999999999999, text_fg = (0, 0, 0, 1), text_align = TextNode.ACenter, text_pos = (0, -0.050000000000000003), pos = (0.82499999999999996, 0, -0.089999999999999997 * i - 0.125)))
    ...
    def initGagFrame(self, toon, expList, meritList):
        ...
        for i in range(len(AvatarDNA.suitDepts)):
            ...
            if CogDisguiseGlobals.isSuitComplete(toon.cogParts, i):
                ...
                meritBar.show()
                meritLabel.show()
                self.meritBarTopLabel.show()
                if totalMerits:
                    meritBar['range'] = totalMerits
                    meritBar['value'] = merits
                    if merits == totalMerits:
                        meritBar['text'] = Localizer.RewardPanelMeritAlert
                    else:
                        meritBar['text'] = '%s/%s' % (merits, totalMerits)
                else:
                    meritBar['range'] = 1
                    meritBar['value'] = 1
                    meritBar['text'] = Localizer.RewardPanelMeritsMaxed
    ...
    def incrementMerits(self, toon, dept, newValue, totalMerits):
        ...
        if totalMerits:
            ...
            if newValue == totalMerits:
                ...
                meritBar['text'] = '%s/%s' % (newValue, totalMerits)
    ...
    def promotion(self, toon, dept):
        ...
        self.itemFrame.hide()
        self.missedItemFrame.hide()
        name = AvatarDNA.suitDepts[dept]
        self.promotionFrame['text'] = 'Ready for promotion in %s track!' % AvatarDNA.suitDeptFullnames[name]
        ...
        for i in range(len(avQuests)):
            ...
            if quest:
                ...
                for cogDict in cogList:
                    num = quest.doesCogCount(avId, cogDict, zoneId, toonList)
                    if num:
                        earnedCogs += num
    ...
    def getExpTrack(self, toon, origExp, earnedExp, deathList, itemList, missedItemList, origMeritList, meritList, partList, toonList):
        ...
        for dept in range(len(AvatarDNA.suitDepts)):
            if meritList[dept]:
