...
class TownBattleSOSPanel(DirectFrame, StateData.StateData):
    ...
    def load(self):
        ...
        self.title = DirectLabel(parent = self, relief = None, text = Localizer.TownBattleSOSNoFriends, text_scale = 0.40000000000000002, text_fg = (1, 1, 1, 1), text_shadow = (0, 0, 0, 1), pos = (0.0, 0.0, 1.45))
    ...
    def makeNPCFriendButton(self, NPCFriendId, numCalls):
        if not Localizer.NPCToonNames.has_key(NPCFriendId):
            return None
        
        friendName = Localizer.NPCToonNames[NPCFriendId]
    ...
    def _TownBattleSOSPanel__updateNPCFriendsPanel(self):
        self.NPCFriends = { }
        for (friend, count) in toonbase.localToon.NPCFriendsDict.items():
