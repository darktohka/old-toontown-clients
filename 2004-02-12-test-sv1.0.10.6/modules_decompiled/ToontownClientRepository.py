...
class ToontownClientRepository(ClientRepository.ClientRepository):
    ...
    def __init__(self, dcFileName, serverVersion, launcher = None):
        ...
        self.parentMgr.registerParent(SPHidden, base.hidden)
        ...
        self.oldShardList = base.config.GetBool('old-shard-list', 0)
    ...
    def gotoFirstScreen(self):
        ...
        if self.isWebPlayToken() or self.isBlue():
            firstScreen = 'login'
        elif newInstall:
    ...
    def enterFreeTimeInform(self):
        ...
        if self.accountOldAuth and self.isPaid() and self.isWebPlayToken() and self.productName == 'Terra-DMC' and self.isBlue():
    ...
    def handleGetShardListResponseMsg(self, di):
        ...
        for i in range(numberOfShards):
            ...
            if self.oldShardList:
                shardWVPop = 0
                shardActive = 1
            else:
                shardWVPop = di.getUint32()
                shardActive = di.getBool()
            if not self._ToontownClientRepository__shards.has_key(shardId):
                shard = PotentialShard.PotentialShard(shardId)
                self._ToontownClientRepository__shards[shardId] = shard
            else:
                shard = self._ToontownClientRepository__shards[shardId]
    ...
    def handleAvatarResponseMsg(self, di):
        ...
        if returnCode == 0:
            ...
            loader.beginBulkLoad('localToonPlayGame', Localizer.TCREnteringToontown, 400, 1, Localizer.TIP_GENERAL)
    ...
    def exitWaitForSetAvatarResponse(self):
        ...
        if self.catalogManager != None:
            self.catalogManager.delete()
            self.catalogManager = None
        ...
        inputState.delete()
        allowedTasks = ('dataloop', 'resetPrevTransform', 'doLaterProcessor', 'eventManager', 'readerPollTask', 'heartBeat', 'igloop', 'collisionloop', 'shadowCollisionLoop', 'ivalloop', 'downloadSequence', 'patchAndHash', 'launcher-download', 'launcher-download-multifile', 'launcher-decompressFile', 'launcher-decompressMultifile', 'launcher-extract', 'launcher-patch', 'tkloop', 'manager-update', 'downloadStallTask', 'irisTask', 'fadeTask')
        problems = []
        ...
        if problems:
            print messenger
            msg = "You can't leave toontown until you clean up your messenger hooks: {"
    ...
    def reachedQuietZone(self):
        self._ToontownClientRepository__gotTimeSync = 0
        self._ToontownClientRepository__cleanupWaitingForDatabase()
        if self.trophyManager != None:
    ...
    def gotTimeSync(self):
        ...
        self._ToontownClientRepository__gotTimeSync = 1
        self.moveOnFromQuietZone()

    
    def moveOnFromQuietZone(self):
        if not (self._ToontownClientRepository__gotTimeSync):
    ...
    def sendSetZoneMsg(self, *args, **kwargs):
        apply(ClientRepository.ClientRepository.sendSetZoneMsg, (self,) + args, kwargs)
    ...
    def _ToontownClientRepository__showWaitingForDatabase(self, task):
        self.notify.info('timed out waiting at %s' % globalClock.getFrameTime())
        self.waitingForDatabase = ToontownDialog.ToontownDialog(text = Localizer.TCRToontownUnavailable, dialogName = 'WaitingForDatabase', buttonTextList = [
            Localizer.TCRToontownUnavailableCancel], style = ToontownDialog.CancelOnly, command = self._ToontownClientRepository__handleCancelWaiting)
