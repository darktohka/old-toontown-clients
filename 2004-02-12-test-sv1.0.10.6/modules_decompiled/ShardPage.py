...
class ShardPage(ShtikerPage.ShtikerPage):
    ...
    def getCurrentShardId(self):
        ...
        if zoneId != None and ZoneUtil.isWelcomeValley(zoneId):
            ...
            return toonbase.localToon.defaultShard

    
    def updateScrollList(self):
        curShardTuples = toonbase.tcr.listActiveShards()
        ...
        if toonbase.tcr.welcomeValleyManager:
            curShardTuples.append((ToontownGlobals.WelcomeValleyToken, Localizer.WelcomeValley[1], 0, 0))
        
        currentShardId = self.getCurrentShardId()
        actualShardId = toonbase.localToon.defaultShard
        ...
        helpText = Localizer.ShardPageHelpIntro
        if actualShardName:
            if currentShardId == ToontownGlobals.WelcomeValleyToken:
                helpText += Localizer.ShardPageHelpWelcomeValley % actualShardName
            else:
                helpText += Localizer.ShardPageHelpWhere % actualShardName
        
        if not (self.book.safeMode):
            helpText += Localizer.ShardPageHelpMove
    ...
    def choseShard(self, shardId):
        zoneId = self.getCurrentZoneId()
        canonicalHoodId = ZoneUtil.getCanonicalHoodId(toonbase.localToon.lastHood)
        currentShardId = self.getCurrentShardId()
