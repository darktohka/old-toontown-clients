...
class PhysicsWalker(DirectObject.DirectObject):
    ...
    def spawnTest(self):
        ...
        duration = distance / 4
        self.moveIval = Sequence(WaitInterval(0.29999999999999999), LerpPosInterval(self.platform, duration, endPos, startPos = startPos, name = 'platformOut%s' % fakeId, fluid = 1), WaitInterval(0.29999999999999999), LerpPosInterval(self.platform, duration, startPos, startPos = endPos, name = 'platformBack%s' % fakeId, fluid = 1), name = 'platformIval%s' % fakeId)
        self.moveIval.loop()
