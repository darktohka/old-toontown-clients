...
class Purchase(PurchaseBase):
    ...
    def load(self):
        ...
        self.playAgain = DirectButton(parent = self.frame, relief = None, scale = 1.04, pos = (0.66000000000000003, 0, -0.23999999999999999), image = (purchaseModels.find('**/PurchScrn_BTN_UP'), purchaseModels.find('**/PurchScrn_BTN_DN'), purchaseModels.find('**/PurchScrn_BTN_RLVR')), text = Localizer.GagShopPlayAgain, text_fg = (0, 0.10000000000000001, 0.69999999999999996, 1), text_scale = 0.050000000000000003, text_pos = (0, 0.014999999999999999, 0), command = self._Purchase__handlePlayAgain)
        self.backToPlayground = DirectButton(parent = self.frame, relief = None, scale = 1.04, pos = (0.66000000000000003, 0, -0.044999999999999998), image = (purchaseModels.find('**/PurchScrn_BTN_UP'), purchaseModels.find('**/PurchScrn_BTN_DN'), purchaseModels.find('**/PurchScrn_BTN_RLVR')), text = Localizer.GagShopBackToPlayground, text_fg = (0, 0.10000000000000001, 0.69999999999999996, 1), text_scale = 0.050000000000000003, text_pos = (0, 0.014999999999999999, 0), command = self._Purchase__handleBackToPlayground)
        ...
        for avInfo in self.avInfoArray:
            av = toonbase.tcr.doId2do.get(avInfo[AVID_INDEX])
            if av:
    ...
    def _Purchase__handlePlayAgain(self):
        ...
        self.statusLabel['text'] = Localizer.GagShopWaitingOtherPlayers
    ...
    def enterReward(self):
        ...
        for index in range(len(self.ids)):
            avId = self.ids[index]
            if self.states[index] != PURCHASE_NO_CLIENT_STATE and self.states[index] != PURCHASE_DISCONNECTED_STATE and avId in toonbase.tcr.doId2do:
                numToons += 1
        ...
        if toonbase.skipMinigameReward:
            self.fsm.request('purchase')
    ...
    def countUp(self):
        ...
        def delayAdd(state):
            ...
            if state.id == toonbase.localToon.doId:
...
class PurchaseHeadFrame(DirectFrame):
    ...
    def setAvatarState(self, state):
        if state == PURCHASE_DISCONNECTED_STATE:
            self.statusLabel['text'] = Localizer.GagShopPlayerDisconnected % self.av.getName()
            self.statusLabel['text_pos'] = (0.014999999999999999, 0.071999999999999995, 0)
            self.head.hide()
            self.tag1.hide()
            self.tag2.hide()
        elif state == PURCHASE_EXIT_STATE:
            self.statusLabel['text'] = Localizer.GagShopPlayerExited % self.av.getName()
            self.statusLabel['text_pos'] = (0.014999999999999999, 0.071999999999999995, 0)
            self.head.hide()
            self.tag1.hide()
            self.tag2.hide()
        elif state == PURCHASE_PLAYAGAIN_STATE:
            self.statusLabel['text'] = Localizer.GagShopPlayerPlayAgain
            self.statusLabel['text_pos'] = (0.10000000000000001, -0.12, 0)
        elif state == PURCHASE_WAITING_STATE:
            self.statusLabel['text'] = Localizer.GagShopPlayerBuying
            self.statusLabel['text_pos'] = (0.10000000000000001, -0.12, 0)
        else:
            raise StandardError
