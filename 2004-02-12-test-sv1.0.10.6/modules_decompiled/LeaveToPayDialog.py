...
class LeaveToPayDialog:
    ...
    def show(self):
        if self.paidUser:
            directFrameText = Localizer.LeaveToSetParentPassword
            directButtonYesText = Localizer.LeaveToSetParentPasswordYes
            directButtonNoText = Localizer.LeaveToSetParentPasswordNo
        else:
            directFrameText = Localizer.LeaveToPay
            directButtonYesText = Localizer.LeaveToPayYes
            directButtonNoText = Localizer.LeaveToPayNo
        if self.dialog == None:
            buttons = loader.loadModelOnce('phase_3/models/gui/dialog_box_buttons_gui')
            okButtonImage = (buttons.find('**/ChtBx_OKBtn_UP'), buttons.find('**/ChtBx_OKBtn_DN'), buttons.find('**/ChtBx_OKBtn_Rllvr'))
            cancelButtonImage = (buttons.find('**/CloseBtn_UP'), buttons.find('**/CloseBtn_DN'), buttons.find('**/CloseBtn_Rllvr'))
            self.dialog = DirectFrame(pos = (0.0, 0.10000000000000001, 0.20000000000000001), relief = None, image = getDefaultDialogGeom(), image_color = ToontownGlobals.GlobalDialogColor, image_scale = (0.90000000000000002, 1.0, 0.5), text = directFrameText, text_align = TextNode.ALeft, text_wordwrap = 14, text_scale = 0.059999999999999998, text_pos = (-0.40000000000000002, 0.14999999999999999), textMayChange = 0)
