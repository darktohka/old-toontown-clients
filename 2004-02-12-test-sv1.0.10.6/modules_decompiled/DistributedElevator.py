...
class DistributedElevator(DistributedObject.DistributedObject):
    ...
    notify.setDebug(1)
    countdownTime = ELEVATOR_COUNTDOWN_TIME
    ...
    def setupElevator(self, big = 0):
        if self.bigElevator == 0:
            self.elevatorSphere = CollisionSphere(0, 5, 0, 5)
        else:
            self.elevatorSphere = CollisionSphere(0, 5, 0, 7.5)
        ...
        self.boardedAvIds = { }
        self.openDoors = getOpenInterval(self, self.leftDoor, self.rightDoor, self.openSfx, self.finalOpenSfx, big)
        self.closeDoors = getCloseInterval(self, self.leftDoor, self.rightDoor, self.closeSfx, self.finalCloseSfx, big)
    ...
    def fillSlot(self, index, avId, x, y, z, h, p, r, timestamp):
        ...
        elif avId == toonbase.localToon.getDoId():
