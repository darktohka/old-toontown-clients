...
class SuitPage(ShtikerPage.ShtikerPage):
    ...
    def load(self):
        ...
        del frameModel
        self.title = DirectLabel(parent = self.iconNode, relief = None, text = Localizer.SuitPageTitle, text_scale = 0.10000000000000001, text_pos = (0.040000000000000001, 0), textMayChange = 0)
        self.radarButtons = []
    ...
    def addQuotaLabel(self, panel):
        ...
        quotaLabel = DirectLabel(parent = panel, pos = (0.0, 0.0, -0.215), relief = None, state = DISABLED, text = Localizer.SuitPageQuota % (count, quota), text_scale = 0.065000000000000002, text_fg = (0, 0, 0, 1), text_font = ToontownGlobals.getSuitFont())
    ...
    def addSuitHead(self, panel, suitName):
        ...
        pos = -0.14000000000000001 + (AvatarDNA.suitsPerDept - column - 1) / 135.0
    ...
    def setPanelStatus(self, panel, status):
        ...
        if status == COG_UNSEEN:
            ...
            if toonbase.localToon.cogs[index] < COG_COMPLETE1:
                ...
                quota = str(COG_QUOTAS[1][index % AvatarDNA.suitsPerDept])
            panel.quotaLabel['text'] = Localizer.SuitPageQuota % (count, quota)
    ...
    def updateCogStatus(self, dept, type, status):
        ...
        elif type < 0 or type > AvatarDNA.suitsPerDept:
    ...
    def updateCogRadar(self, deptNum, panels, timeout = 0):
        ...
        for panel in panels:
            panel.cogRadarLabel['text'] = Localizer.SuitPageCogRadar % panel.count
            if self.radarOn[deptNum]:
                panel.quotaLabel.hide()
    ...
    def updateBuildingRadar(self, deptNum, timeout = 0):
        ...
        if button.building:
            ...
            if self.radarOn[deptNum]:
                ...
                if num == 1:
                    button.buildingRadarLabel['text'] = Localizer.SuitPageBuildingRadarS % num
                else:
                    button.buildingRadarLabel['text'] = Localizer.SuitPageBuildingRadarP % num
                
                def showLabel(button):
                    button.buildingRadarLabel.show()
                    button['state'] = NORMAL

                taskMgr.doMethodLater(RADAR_DELAY * AvatarDNA.suitsPerDept, showLabel, 'showBuildingRadarLater', extraArgs = (button,))
            else:
                button.buildingRadarLabel.hide()
