...
class LOrientationf(Quat.Quat, FFIExternalObject.FFIExternalObject):
    ...
    def _LOrientationf__overloaded___mul___ptrConstLOrientationf_ptrConstLOrientationf(self, other):
        returnValue = libpanda._inPVZN3Km99(self.this, other.this)
    ...
    def __mul__(self, *_args):
        numArgs = len(_args)
        if numArgs == 1:
            if isinstance(_args[0], LOrientationf):
                return self._LOrientationf__overloaded___mul___ptrConstLOrientationf_ptrConstLOrientationf(*_args)
            
            import Quat
            if isinstance(_args[0], Quat.Quat):
                return self._LOrientationf__overloaded___mul___ptrConstLOrientationf_ptrConstLQuaternionf(*_args)
            
            raise TypeError, 'Invalid argument 0, expected one of: <LOrientationf> <Quat.Quat> '
        else:
