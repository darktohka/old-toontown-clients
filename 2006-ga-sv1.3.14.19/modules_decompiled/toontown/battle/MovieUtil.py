...
def createSuitDeathTrack(suit, toon, battle, npcs = []):
    ...
    gears1Track = Sequence(Wait(2.1000000000000001), ParticleInterval(smallGears, battle, worldRelative = 0, duration = 4.2999999999999998), name = 'gears1Track')
    gears2MTrack = Track((0.0, explosionTrack), (0.69999999999999996, ParticleInterval(singleGear, battle, worldRelative = 0, duration = 5.7000000000000002)), (5.2000000000000002, ParticleInterval(smallGearExplosion, battle, worldRelative = 0, duration = 1.2)), (5.4000000000000004, ParticleInterval(bigGearExplosion, battle, worldRelative = 0, duration = 1.0)), name = 'gears2MTrack')
...
def getToonTeleportOutInterval(toon):
    ...
    runTrack = Sequence(ActorInterval(toon, 'teleport', duration = dur), Wait(T_HOLE_CLOSES), Func(toon.reparentTo, hidden))
...
def getToonTeleportInInterval(toon):
    ...
    holeAnimTrack = Sequence()
    holeAnimTrack.append(Func(toon.reparentTo, hidden))
...
def indicateMissed(actor, duration = 1.1000000000000001, scale = 0.69999999999999996):
    actor.showLaffString(TTLocalizer.AttackMissed, duration = duration, scale = scale)


def createKapowExplosionTrack(parent, explosionPoint = None):
    explosionTrack = Sequence()
    ...
    explosionTrack.append(Func(explosion.setScale, 0.40000000000000002))
