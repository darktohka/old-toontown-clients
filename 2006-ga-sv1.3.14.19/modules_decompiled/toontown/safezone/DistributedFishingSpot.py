...
class DistributedFishingSpot(DistributedObject.DistributedObject):
    ...
    def _DistributedFishingSpot__makeGui(self):
        ...
        self.itemBoot.hide()
        self.itemPackage.hide()
        self.failureDialog = TTDialog.TTGlobalDialog(dialogName = self.uniqueName('failureDialog'), doneEvent = self.uniqueName('failureDialog'), command = self._DistributedFishingSpot__userExit, message = TTLocalizer.FishingFailure, style = TTDialog.CancelOnly, cancelButtonText = TTLocalizer.FishingExit, text_scale = TTLocalizer.DFSfailureDialog)
        self.failureDialog.hide()
