...
class BingoCardGui(DirectFrame):
    ...
    def __init__(self, parent = aspect2d, **kw):
        ...
        self.model = loader.loadModelOnce('phase_4/models/gui/FishBingo')
        optiondefs = (('relief', None, None), ('state', NORMAL, None), ('image', self.model.find('**/g'), None), ('image_color', BG.getColor(0), None), ('image_scale', BG.CardImageScale, None), ('image_hpr', (0.0, 90.0, 0.0), None), ('pos', BG.CardPosition, None))
    ...
    def setJackpotText(self, text):
        if text:
            str = TTLocalizer.FishBingoJackpot % text
        else:
            str = ''
    ...
    def loadNextGameTimer(self):
        self.nextGame = DirectLabel(parent = self, relief = None, text = '', text_font = ToontownGlobals.getBuildingNametagFont(), text_scale = 1.71 * BG.CardImageScale[2], text_fg = (1.0, 1.0, 1.0, 1), pos = (BG.GridXOffset, 0, 4 * BG.CardImageScale[2]))
    ...
    def load(self):
        ...
        self.bingo = DirectButton(parent = self, pos = (BG.GridXOffset, 0, 0.30499999999999999), scale = (0.034299999999999997, 0.035000000000000003, 0.035000000000000003), relief = None, state = DISABLED, geom = self.model.find('**/BINGObutton'), geom_pos = (0, 0, 0), geom_hpr = (0, 90, 0), image = (self.model.find('**/Gold_TTButtUP'), self.model.find('**/goldTTButtDown'), self.model.find('**/RolloverBingoButton1')), image_pos = (0, 0, 0), image_hpr = (0, 90, 0), image_color = BG.getButtonColor(0), pressEffect = False)
        guiButton.removeNode()
        arrowModel = loader.loadModelOnce('phase_3.5/models/gui/speedChatGui')
        self.gameType = DirectButton(parent = self, pos = (BG.GridXOffset, 0, -8 * BG.CardImageScale[2] - 0.01), relief = None, image = arrowModel.find('**/chatArrow'), image_scale = -0.050000000000000003, image_pos = (-0.20000000000000001, 0, 0.025000000000000001), text = '', text_scale = 0.044999999999999998, text_fg = (1, 1, 1, 1), text_font = ToontownGlobals.getBuildingNametagFont(), text_wordwrap = 10.5, text_pos = (0.01, 0.0080000000000000002), pressEffect = False)
        arrowModel.removeNode()
        self.gameType.bind(ENTER, self.onMouseEnter)
        self.gameType.bind(EXIT, self.onMouseLeave)
        self.gameType.hide()
        self.jpText = DirectLabel(parent = self, pos = (BG.GridXOffset, 0, 0.22), relief = None, state = NORMAL, text = '', text_scale = TTLocalizer.BCGjpText, text_pos = (0, 0, 0), text_fg = (1, 1, 1, 1), text_shadow = (0, 0, 0, 1), text_font = ToontownGlobals.getSignFont(), text_wordwrap = TTLocalizer.BCGjpTextWordwrap)
        self.gameOver = DirectLabel(parent = self, pos = (BG.GridXOffset, 0, 0), relief = None, state = NORMAL, text = '', text_scale = textScale, text_fg = (1, 1, 1, 1), text_font = ToontownGlobals.getBuildingNametagFont())
        self.jpSign = DirectFrame(parent = self.parent, relief = None, state = NORMAL, pos = BG.CardPosition, scale = (0.035000000000000003, 0.035000000000000003, 0.035000000000000003), text = TTLocalizer.FishBingoJackpot, text_scale = 2, text_pos = (-1.5, 18.600000000000001), text_fg = (1, 1, 1, 1), image = self.model.find('**/jackpot'), image_pos = (0, 0, 0), image_hpr = (0, 90, 0), sortOrder = BACKGROUND_SORT_INDEX)
