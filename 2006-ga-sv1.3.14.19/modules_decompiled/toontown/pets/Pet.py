...
class Pet(Avatar.Avatar):
    ...
    def generatePet(self):
        ...
        self.loadAnims({
            'toBeg': 'phase_5.5/models/char/TT_pets-intoBeg',
            'beg': 'phase_5.5/models/char/TT_pets-beg',
            'fromBeg': 'phase_5.5/models/char/TT_pets-begOut',
            'backflip': 'phase_5.5/models/char/TT_pets-backflip',
            'dance': 'phase_5.5/models/char/TT_pets-heal',
            'toDig': 'phase_5.5/models/char/TT_pets-intoDig',
            'dig': 'phase_5.5/models/char/TT_pets-dig',
            'fromDig': 'phase_5.5/models/char/TT_pets-digToNeutral',
            'disappear': 'phase_5.5/models/char/TT_pets-disappear',
            'eat': 'phase_5.5/models/char/TT_pets-eat',
            'jump': 'phase_5.5/models/char/TT_pets-jump',
            ...
            'playDead': 'phase_5.5/models/char/TT_pets-playdead',
            'fromPlayDead': 'phase_5.5/models/char/TT_pets-deadend',
            'reappear': 'phase_5.5/models/char/TT_pets-reappear',
            'run': 'phase_5.5/models/char/TT_pets-run',
            'rollover': 'phase_5.5/models/char/TT_pets-rollover',
            'walkSad': 'phase_5.5/models/char/TT_pets-sadwalk',
            'speak': 'phase_5.5/models/char/TT_pets-speak',
        ...
        self.rightPupil.setColor(eyeColor)
        self.leftPupil.setColor(eyeColor)
    ...
    def generateMoods(self):
        moodIcons = loader.loadModelCopy('phase_4/models/char/petEmotes')
    ...
    def getNametagJoints(self):
        joints = []
        char = self.getPart('modelRoot').node()
        joint = char.getBundle().findChild('attachNametag')
        if joint:
            joints.append(joint)
    ...
    def enterTeleportIn(self, timestamp):
        ...
        self.track = Sequence(Wait(1.0), Parallel(self.getTeleportInSoundInterval(), Sequence(Func(self.showHole), ActorInterval(self.teleportHole, 'hole', startFrame = 81, endFrame = 71), ActorInterval(self, 'reappear'), ActorInterval(self.teleportHole, 'hole', startFrame = 71, endFrame = 81), Func(self.cleanupHole), Func(self.loop, 'neutral'))))
    ...
    def enterTeleportOut(self, timestamp):
        ...
        self.track = Sequence(Wait(1.0), Parallel(self.getTeleportOutSoundInterval(), Sequence(ActorInterval(self, 'toDig'), Parallel(ActorInterval(self, 'dig'), Func(self.showHole), ActorInterval(self.teleportHole, 'hole', startFrame = 81, endFrame = 71)), ActorInterval(self, 'disappear'), ActorInterval(self.teleportHole, 'hole', startFrame = 71, endFrame = 81), Func(self.cleanupHole))))
        self.track.start(globalClockDelta.localElapsedTime(timestamp))
    ...
    def getTeleportInSoundInterval(self):
        if not (self.soundTeleportIn):
            self.soundTeleportIn = loader.loadSfx('phase_5.5/audio/sfx/teleport_reappear.mp3')
    ...
    def getTeleportOutSoundInterval(self):
        if not (self.soundTeleportOut):
            self.soundTeleportOut = loader.loadSfx('phase_5.5/audio/sfx/teleport_disappear.mp3')
    ...
    def _Pet__blinkClosed(self, task):
        self.eyesClose()
        taskMgr.doMethodLater(0.125, self._Pet__blinkOpen, self._Pet__blinkName)
        return Task.done
