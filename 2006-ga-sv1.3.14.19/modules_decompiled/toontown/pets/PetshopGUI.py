...
0]))
    ...
    class NamePicker(DirectFrame):
        ...
        def __init__(self, doneEvent, petSeed, gender):
            ...
            self.submitButton = DirectButton(parent = self, relief = None, image = (self.gui.find('**/SubmitUpButton'), self.gui.find('**/SubmitDownButton'), self.gui.find('**/SubmitRolloverButton')), scale = self.guiScale, text = TTLocalizer.PetshopAdopt, text_pos = (3.2999999999999998, -5.7000000000000002), text_scale = TTLocalizer.PGUIadoptSubmit, text_fg = text0Color, pressEffect = False, command = lambda : messenger.send(doneEvent, [
...
-1]))
    ...
    class MainMenuDlg(DirectFrame):
        ...
        def __init__(self, doneEvent):
            ...
            self.cancelButton = DirectButton(parent = self, relief = None, scale = (modelScale, modelScale, modelScale), geom = XImageList, image = cancelImageList, text = ('', TTLocalizer.PetshopCancel), text_pos = TTLocalizer.PGUIpetshopCancelPos, text_scale = 0.80000000000000004, pressEffect = False, command = lambda : messenger.send(doneEvent, [
0]))
    ...
    class ReturnPetDlg(DirectFrame):
        ...
        def __init__(self, doneEvent):
            ...
            def showDialog(avatar):
                ...
                base.r = self
                DirectFrame.__init__(self, relief = None, state = 'normal', geom = model, geom_scale = modelScale, frameSize = (-1, 1, -1, 1), pos = modelPos, text = TTLocalizer.PetshopReturnConfirm % avatar.getName(), text_wordwrap = 12, text_scale = TTLocalizer.PGUIreturnConfirm, text_pos = (0, 0.45000000000000001), text_fg = text2Color)
...
-1]))
            ...
            self.okButton = DirectButton(parent = self, relief = None, image = adoptImageList, image3_color = disabledImageColor, scale = modelScale, text = TTLocalizer.PetshopAdopt, text_scale = TTLocalizer.PGUIpetsopAdopt, text_pos = TTLocalizer.PGUIpetsopAdoptPos, text0_fg = text0Color, text1_fg = text1Color, text2_fg = text2Color, text3_fg = text3Color, pressEffect = False, command = lambda : messenger.send(doneEvent, [
self.curPet]))
        ...
        def showPet(self):
            ...
            self.petModel.setH(130)
            self.petModel.enterNeutralHappy()
            self.descLabel = DirectLabel(parent = self, pos = (-0.40000000000000002, 0, 0.71999999999999997), relief = None, scale = 0.050000000000000003, text = self.petDesc[self.curPet], text_align = TextNode.ALeft, text_wordwrap = TTLocalizer.PGUIwordwrap, text_scale = TTLocalizer.PGUIdescLabel)
