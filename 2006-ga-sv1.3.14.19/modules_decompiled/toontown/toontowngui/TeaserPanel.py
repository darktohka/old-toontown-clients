...
Pages = {
    ...
    'mailers': (TTLocalizer.TeaserCardsAndPosters, 'features-mailers', 0, 1),
    'holidays': (TTLocalizer.TeaserHolidays, 'features-holidays', 0, 0),
    'quests': (TTLocalizer.TeaserQuests, 'features-quests', 0, 0),
    'emotions': (TTLocalizer.TeaserEmotions, 'features-catalog', 0, 0),
...
class TeaserPanel(DirectObject):
    ...
    def showPage(self, pageName):
        ...
        self.cleanup()
        self.dialog = TTDialog.TTDialog(text = TTLocalizer.TeaserTop, text_wordwrap = 20, text_scale = TTLocalizer.TPtop, topPad = -0.050000000000000003, midPad = 1.25, sidePad = 0.0, command = self._TeaserPanel__handleDone, fadeScreen = 0.5, style = TTDialog.TwoChoice, buttonTextList = [
            TTLocalizer.TeaserSubscribe,
            TTLocalizer.TeaserContinue], button_text_scale = TTLocalizer.TPbutton, buttonPadSF = 5.5, sortOrder = NO_FADE_SORT_INDEX)
...
class FeatureBrowser(DirectScrolledList):
    ...
    def load(self):
        ...
        for page in PageOrder:
            ...
            panel = DirectFrame(parent = self, relief = None, image = imageModel, image_scale = scale, image_pos = (0, 0, 0.050000000000000003), text = textInfo, text_scale = TTLocalizer.TPpanel, text_pos = (0, -0.55000000000000004))
            self.addItem(panel)
            if members:
