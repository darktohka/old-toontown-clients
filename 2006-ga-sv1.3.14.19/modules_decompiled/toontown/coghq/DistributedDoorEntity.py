...
from direct.level import VisibilityBlocker
...
class DistributedDoorEntity(DistributedDoorEntityBase.DistributedDoorEntityBase, DistributedEntity.DistributedEntity, BasicEntities.NodePathAttribsProxy, FourState.FourState, VisibilityBlocker.VisibilityBlocker):
    ...
    def openInnerDoors(self):
        if self.isOuterDoorOpen and not (self.isVisBlocker) or self.isVisReady:
            ...
            moveDistance = 8.0
            self.setInnerDoorsTrack(Sequence(Func(self.leftInnerCollision.unstash), Func(self.rightInnerCollision.unstash), Parallel(SoundInterval(slideSfx, node = self.node, duration = duration * 0.40000000000000002, volume = 0.80000000000000004), LerpPosInterval(nodePath = self.doorLeft, duration = duration * 0.40000000000000002, pos = Vec3(-moveDistance, 0.0, 0.0), blendType = 'easeOut'), LerpPosInterval(nodePath = self.doorRight, duration = duration * 0.40000000000000002, pos = Vec3(moveDistance, 0.0, 0.0), blendType = 'easeOut'), Sequence(Wait(duration * 0.375), SoundInterval(finalSfx, node = self.node, duration = 1.0, volume = 0.80000000000000004))), Func(self.doorLeft.stash), Func(self.doorRight.stash)))
    ...
    def closeInnerDoors(self):
        ...
        finalSfx = base.loadSfx('phase_9/audio/sfx/CHQ_FACT_door_open_final.mp3')
        moveDistance = 8.0
        self.setInnerDoorsTrack(Sequence(Func(self.doorLeft.unstash), Func(self.doorRight.unstash), Parallel(SoundInterval(slideSfx, node = self.node, duration = duration * 0.40000000000000002, volume = 0.80000000000000004), LerpPosInterval(nodePath = self.doorLeft, duration = duration * 0.40000000000000002, pos = Vec3(0.0), blendType = 'easeIn'), LerpPosInterval(nodePath = self.doorRight, duration = duration * 0.40000000000000002, pos = Vec3(0.0), blendType = 'easeIn'), Sequence(Wait(duration * 0.375), SoundInterval(finalSfx, node = self.node, duration = 1.0, volume = 0.80000000000000004))), Func(self.leftInnerCollision.stash), Func(self.rightInnerCollision.stash)))
    ...
    def enterState1(self):
        ...
        finalSfx = base.loadSfx('phase_9/audio/sfx/CHQ_FACT_door_open_final.mp3')
        moveDistance = 8.0
        self.setTrack(Sequence(Wait(duration * 0.10000000000000001), Parallel(SoundInterval(slideSfx, node = self.node, duration = duration * 0.40000000000000002, volume = 0.80000000000000004), LerpPosInterval(nodePath = self.doorTop, duration = duration * 0.40000000000000002, pos = Vec3(0.0, 0.0, moveDistance), blendType = 'easeOut'), LerpPosInterval(nodePath = self.doorBottom, duration = duration * 0.40000000000000002, pos = Vec3(0.0, 0.0, -moveDistance), blendType = 'easeOut'), Sequence(Wait(duration * 0.375), SoundInterval(finalSfx, node = self.node, duration = 1.0, volume = 0.80000000000000004))), Func(self.doorTop.stash), Func(self.doorBottom.stash), Func(self.setisOuterDoorOpen, 1), Func(self.openInnerDoors)))
    ...
    def enterState3(self):
        ...
        slideSfx = base.loadSfx('phase_9/audio/sfx/CHQ_FACT_door_open_sliding.mp3')
        finalSfx = base.loadSfx('phase_9/audio/sfx/CHQ_FACT_door_open_final.mp3')
        self.setTrack(Sequence(Wait(duration * 0.10000000000000001), Func(self.closeInnerDoors), Wait(duration * 0.40000000000000002), Func(self.doorTop.unstash), Func(self.doorBottom.unstash), Parallel(SoundInterval(slideSfx, node = self.node, duration = duration * 0.40000000000000002, volume = 0.80000000000000004), LerpPosInterval(nodePath = self.doorTop, duration = duration * 0.40000000000000002, pos = Vec3(0.0), blendType = 'easeIn'), LerpPosInterval(nodePath = self.doorBottom, duration = duration * 0.40000000000000002, pos = Vec3(0.0), blendType = 'easeIn'), Sequence(Wait(duration * 0.375), SoundInterval(finalSfx, node = self.node, duration = duration * 0.40000000000000002, volume = 0.80000000000000004))), Func(self.setisOuterDoorOpen, 0)))
