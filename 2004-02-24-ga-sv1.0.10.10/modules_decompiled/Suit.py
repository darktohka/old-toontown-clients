...
def loadSkelDialog():
    ...
    if len(SkelSuitDialogArray) > 0:
        ...
        grunt = loader.loadSfx('phase_9/audio/sfx/Skel_COG_VO_grunt.mp3')
        murmur = loader.loadSfx('phase_9/audio/sfx/Skel_COG_VO_murmur.mp3')
        statement = loader.loadSfx('phase_9/audio/sfx/Skel_COG_VO_statement.mp3')
        question = loader.loadSfx('phase_9/audio/sfx/Skel_COG_VO_question.mp3')
        SkelSuitDialogArray = [
            grunt,
            murmur,
            statement,
...
class Suit(Avatar.Avatar):
    ...
    def getLoseActor(self):
        if self.loseActor == None:
            if not (self.isSkeleton):
                ...
                loseModel = 'phase_9/models/char/cog' + string.upper(self.style.body) + '_robot-lose-mod'
                (filePrefix, phase) = ModelDict[self.style.body]
                loseAnim = 'phase_' + str(phase) + filePrefix + 'lose'
                self.loseActor = Actor.Actor(loseModel, {
                    'lose': loseAnim })
    ...
    def makeSkeleton(self):
        model = 'phase_9/models/char/cog' + string.upper(self.style.body) + '_robot-zero'
        (filePrefix, phase) = ModelDict[self.style.body]
        anims = self.generateAnimDict(filePrefix, phase)
        anim = self.getCurrentAnim()
        dropShadow = self.find('**/drop_shadow.egg')
        ...
        self.setName('Skeleton')
        nameInfo = Localizer.SuitBaseNameWithLevel % {
            'name': self.name,
            'dept': self.getStyleDept(),
            'level': self.getActualLevel() }
        self.setDisplayName(nameInfo)
        dropShadow.setScale(0.75)
        dropShadow.reparentTo(self.find('**/joint-shadow'))
        self.leftHand = self.find('**/joint-Lhold')
        self.rightHand = self.find('**/joint-Rhold')
