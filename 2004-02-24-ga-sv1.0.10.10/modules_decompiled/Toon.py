...
class Toon(Avatar.Avatar, ToonHead):
    ...
    def __init__(self):
        ...
        self.animFSM = FSM.FSM('Toon', [
            ...
            State.State('jumpSquat', self.enterJumpSquat, self.exitJumpSquat),
            State.State('jump', self.enterJump, self.exitJump),
            State.State('jumpAirborne', self.enterJumpAirborne, self.exitJumpAirborne),
            State.State('jumpLand', self.enterJumpLand, self.exitJumpLand),
            ...
            State.State('FallDown', self.enterFallDown, self.exitFallDown)], 'off', 'off')
        ...
        if self.hasLOD():
            if self.style.getGender() == 'f' and fromNet == 0:
                ...
                try:
                    ...
                    bottomPair = AvatarDNA.GirlBottoms[0]
            ...
            for lodName in self.getLODNames():
                ...
                try:
                    ...
                    color = AvatarDNA.ClothesColors[0]
                ...
                if self.style.getGender() == 'm':
                    ...
                    try:
                        ...
                        texName = AvatarDNA.GirlBottoms[0][0]
    ...
    def enterSad(self, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
        ...
        self.setSpeed(self.forwardSpeed, self.rotateSpeed)
        Emote.DisableBody(self, 'toon, enterSad')
        self.setActiveShadow(1)
    ...
    def exitSad(self):
        ...
        self.stop()
        self.motion.exit()
        Emote.ReleaseBody(self, 'toon, exitSad')
    ...
    def enterSwim(self, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
        Emote.DisableAll(self, 'enterSwim')
    ...
    def exitSwim(self):
        ...
        Emote.ReleaseAll(self, 'exitSwim')
    ...
    def enterOpenBook(self, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
        Emote.DisableAll(self, 'enterOpenBook')
    ...
    def exitOpenBook(self):
        ...
        self.hideBooks()
        self.startLookAround()
        Emote.ReleaseAll(self, 'exitOpenBook')

    
    def enterReadBook(self, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
        Emote.DisableBody(self, 'enterReadBook')
    ...
    def exitReadBook(self):
        ...
        self.startLookAround()
        Emote.ReleaseBody(self, 'exitReadBook')

    
    def enterCloseBook(self, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
        Emote.DisableAll(self, 'enterCloseBook')
    ...
    def exitCloseBook(self):
        ...
        Emote.ReleaseAll(self, 'exitCloseBook')
    ...
    def enterTeleportOut(self, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
        ...
        Emote.DisableAll(self, 'enterTeleportOut')
        if self.isLocal():
            autoFinishTrack = 0
        else:
            autoFinishTrack = 1
    ...
    def exitTeleportOut(self):
        ...
        Emote.ReleaseAll(self, 'exitTeleportOut')
        self.show()
    ...
    def getDiedInterval(self, autoFinishTrack = 1):
        ...
        if hasattr(self, 'uniqueName'):
            ...
            trackName = 'died'
        ival = Sequence(Func(Emote.DisableBody, self), Func(self.sadEyes), Func(self.blinkEyes), Track((0, ActorInterval(self, 'lose')), (2, SoundInterval(sound, node = self)), (5.3330000000000002, self.scaleInterval(1.5, VBase3(0.01, 0.01, 0.01), blendType = 'easeInOut'))), Func(self.detachNode), Func(self.setScale, 1, 1, 1), Func(self.normalEyes), Func(self.blinkEyes), Func(Emote.ReleaseBody, self), name = trackName, autoFinish = autoFinishTrack)
        return ival
    ...
    def enterDied(self, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
        ...
        Emote.DisableAll(self, 'enterDied')
        if self.isLocal():
            autoFinishTrack = 0
        else:
            autoFinishTrack = 1
        self.track = self.getDiedInterval(autoFinishTrack)
        self.diedCallback = callback
        self.diedCallbackArgs = extraArgs
        self.track.setDoneEvent(self.track.getName())
        self.acceptOnce(self.track.getName(), self.finishDied, [
            callback,
            extraArgs])
        self.track.start(ts)
        self.setActiveShadow(0)
    ...
    def exitDied(self):
        ...
        Emote.ReleaseAll(self, 'exitDied')
        self.show()
    ...
    def enterTeleportIn(self, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
        ...
        self.playingAnim = 'teleport'
        Emote.DisableAll(self, 'enterTeleportIn')
    ...
    def exitTeleportIn(self):
        ...
        Emote.ReleaseAll(self, 'exitTeleportIn')
    ...
    def stunToon(self, ts = 0, callback = None, knockdown = 0):
        if not (self.isStunned):
            ...
            origColorScale = None
            if node.hasColorScale():
                origColorScale = node.getColorScale()
            
            lerpTime = 0.5
            down = self._Toon__doToonColorScale(VBase4(1, 1, 1, 0.59999999999999998), lerpTime)
            up = self._Toon__doToonColorScale(VBase4(1, 1, 1, 0.90000000000000002), lerpTime)
            clear = self._Toon__doToonColorScale(origColorScale, lerpTime)
            track = Sequence(Func(setStunned, 1), down, up, down, up, down, up, down, clear, Func(setStunned, 0))
            if knockdown:
    ...
    def putOnSuit(self, suitType):
        ...
        suitDept = AvatarDNA.suitDepts.index(AvatarDNA.getSuitDept(suitType))
        suitName = SuitBattleGlobals.SuitAttributes[suitType]['name']
        self.setDisplayName(Localizer.SuitBaseNameWithLevel % {
            'name': self.getName(),
            'dept': suitName,
        ...
        if self.isLocal():
            ...
            indices = range(Localizer.SCMenuCommonCogIndices[0], Localizer.SCMenuCommonCogIndices[1] + 1)
            customIndices = Localizer.SCMenuCustomCogIndices[suitType]
    ...
    def takeOffSuit(self):
        ...
        Emote.ReleaseAll(self)
        self.isDisguised = 0
        self.setFont(ToontownGlobals.getInterfaceFont())
    ...
    def getPresentPieInterval(self, x, y, z, h, p, r):
        ...
        if pieType == 'actor':
            ...
            pingpongPie = Func(pie.pingpong, pieName, 32, 47)
        
        track = Sequence(Func(self.setPosHpr, x, y, z, h, p, r), Func(pie.reparentTo, self.rightHand), Func(pie.setPosHpr, 0, 0, 0, 0, 0, 0), Parallel(pie.scaleInterval(1, pie.getScale(), startScale = MovieUtil.PNT3_NEARZERO), ActorInterval(self, 'throw', startFrame = 0, endFrame = 31), animPie), Func(self.pingpong, 'throw', 32, 47), pingpongPie)
        return track
    ...
    def getTossPieInterval(self, x, y, z, h, p, r, power, beginFlyIval = Sequence()):
        ...
        fly = Track((14.0 / 24.0, SoundInterval(sound, node = self)), (16.0 / 24.0, Sequence(Func(flyPie.reparentTo, render), Func(flyPie.setPosHpr, self, 0.52000000000000002, 0.96999999999999997, 2.2400000000000002, 1.95, -89.430000000000007, -79.420000000000002), beginFlyIval, ProjectileInterval(flyPie, startVel = getVelocity, duration = 3), Func(flyPie.detachNode))))
