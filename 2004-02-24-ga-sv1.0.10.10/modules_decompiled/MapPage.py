...
class MapPage(ShtikerPage.ShtikerPage):
    ...
    def load(self):
        ...
        self.cloudScaleList = (((0.55000000000000004, 0, 0.40000000000000002), (0.34999999999999998, 0, 0.25)), (), ((0.45000000000000001, 0, 0.45000000000000001), (0.5, 0, 0.40000000000000002)), ((0.86645000000000005, 0, 0.45500000000000002),), ((0.55000000000000004, 0, 0.40000000000000002),), ((0.59999999999999998, 0, 0.40000000000000002), (0.53320000000000001, 0, 0.32000000000000001)), ((0.69999999999999996, 0, 0.45000000000000001),), ((0.69999999999999996, 0, 0.45000000000000001),), ((0.79979999999999996, 0, 0.39000000000000001),), ((-0.45000000000000001, 0, 0.34999999999999998),), ((-0.45000000000000001, 0, 0.40000000000000002),), ((0.5, 0, 0.34999999999999998),), ((0.5, 0, 0.40000000000000002),))
        self.cloudPosList = (((0.57499999999999996, 0.0, -0.040000000000000001), (0.45000000000000001, 0.0, -0.25)), (), ((0.375, 0.0, 0.40000000000000002), (0.5625, 0.0, 0.20000000000000001)), ((-0.062399999999999997, 0.0, 0.23000000000000001),), ((-0.29999999999999999, 0.0, -0.40000000000000002),), ((0.25, 0.0, -0.42499999999999999), (0.125, 0.0, -0.35999999999999999)), ((-0.45000000000000001, 0.0, 0.21249999999999999),), ((-0.5625, 0.0, -0.070000000000000007),), ((-0.125, 0.0, 0.5),), ((-0.59999999999999998, 0.0, 0.45000000000000001),), ((-0.68000000000000005, 0.0, -0.44400000000000001),), ((0.66000000000000003, 0.0, 0.5),), ((0.66000000000000003, 0.0, -0.40000000000000002),))
    ...
    def enter(self):
        ...
        if toonbase.localToon.lastHood >= ToontownGlobals.BossbotHQ:
            self.safeZoneButton['text'] = Localizer.MapPageBackToCogHQ
        else:
            self.safeZoneButton['text'] = Localizer.MapPageBackToPlayground
        if zone and ZoneUtil.isPlayground(zone) or self.book.safeMode:
            self.safeZoneButton.hide()
        else:
            self.safeZoneButton.show()
        if toonbase.tcr.playGame.getPlaceId() == ToontownGlobals.MyEstate and toonbase.tcr.playGame.hood.loader.atMyEstate() or self.book.safeMode:
            self.goHomeButton.hide()
        elif toonbase.housingEnabled:
            self.goHomeButton.show()
        
        if toonbase.tcr.playGame.getPlaceId() == ToontownGlobals.MyEstate:
            if toonbase.tcr.playGame.hood.loader.atMyEstate():
                self.hoodLabel['text'] = Localizer.MapPageYouAreAtHome
                self.hoodLabel.show()
            else:
                avatar = toonbase.tcr.identifyAvatar(toonbase.tcr.playGame.hood.loader.estateOwnerId)
                if avatar:
        ...
        if toonbase.localToon.teleportCheat:
            ...
            safeZonesVisited = toonbase.localToon.hoodsVisited
    ...
    def _MapPage__buttonCallback(self, hood):
        if toonbase.localToon.teleportCheat and hood in toonbase.tcr.playGame.Hood2StateDict.keys() or hood in toonbase.localToon.getTeleportAccess():
            self.doneStatus = {
                'mode': 'teleport',
                'hood': hood }
            messenger.send(self.doneEvent)
