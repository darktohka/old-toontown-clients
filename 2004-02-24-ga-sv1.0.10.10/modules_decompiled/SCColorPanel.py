...
def scRgbPanel(callback, title, initColor):
    ...
    def getDefaultFrameColor():
        toonbase.speedChat.setColorScheme(SCColorScheme(arrowColor = colors['arrowColor'], rolloverColor = colors['rolloverColor']))
        colors['frameColor'] = toonbase.speedChat.getColorScheme().getFrameColor()
    ...
    def updateAllPanels():
        cs = toonbase.speedChat.getColorScheme()
...
def adjustSCColors():
    ...
    for colorName in colors.keys():
        ...
        def handleCallback(color, colorName = colorName):
            ...
            toonbase.speedChat.setColorScheme(SCColorScheme(**None))
