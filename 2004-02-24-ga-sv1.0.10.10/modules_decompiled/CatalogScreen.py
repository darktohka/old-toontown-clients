...
class CatalogScreen(DirectFrame):
    ...
    def showPageItems(self):
        ...
        randGen.seed(toonbase.localToon.catalogScheduleCurrentWeek + (self.pageIndex << 8) + (newOrBack << 16))
    ...
    def load(self, guiItems):
        ...
        seriesNumber = (toonbase.localToon.catalogScheduleCurrentWeek - 1) / ToontownGlobals.CatalogNumWeeksPerSeries + 1
        weekNumber = (toonbase.localToon.catalogScheduleCurrentWeek - 1) % ToontownGlobals.CatalogNumWeeksPerSeries + 1
        geom = NodePath('cover')
        cover = guiItems.find('**/cover')
        coverPicture = cover.find('**/cover_picture%s' % seriesNumber)
        ...
        if toonbase.localToon.catalogScheduleCurrentWeek <= ToontownGlobals.CatalogNumWeeks:
            catalogNumber = DirectLabel(self.cover, relief = None, scale = 0.20000000000000001, pos = (-0.22, 0, -0.33000000000000002), text = '#%d' % weekNumber, text_fg = (0.94999999999999996, 0.94999999999999996, 0, 1), text_shadow = (0, 0, 0, 1), text_font = ToontownGlobals.getInterfaceFont())
            catalogSeries = DirectLabel(self.cover, relief = None, scale = (0.22, 1, 0.17999999999999999), pos = (-0.76000000000000001, 0, -0.90000000000000002), text = Localizer.CatalogSeriesLabel % seriesNumber, text_fg = (0.90000000000000002, 0.90000000000000002, 0.40000000000000002, 1), text_shadow = (0, 0, 0, 1), text_font = ToontownGlobals.getInterfaceFont())
            catalogSeries.setShxz(0.40000000000000002)
    ...
    def hangUp(self):
        self.setClarabelleChat(random.choice(Localizer.CatalogGoodbyeList))
        self.setPageIndex(-1)
        self.showPageItems()
        self.nextPageButton.hide()
        self.backPageButton.hide()
