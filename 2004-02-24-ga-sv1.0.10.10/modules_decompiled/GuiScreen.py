...
class GuiScreen:
    ...
    def waitForDatabaseTimeout(self):
        globalClock.tick()
        taskMgr.doMethodLater(DatabaseDialogTimeout, self._GuiScreen__showWaitingForDatabase, 'waitingForDatabase')

    
    def _GuiScreen__showWaitingForDatabase(self, task):
        self.waitingForDatabase = ToontownDialog.ToontownDialog(text = Localizer.GuiScreenToontownUnavailable, dialogName = 'WaitingForDatabase', buttonTextList = [
            Localizer.GuiScreenCancel], style = ToontownDialog.Acknowledge, command = self._GuiScreen__handleCancelWaiting)
        self.waitingForDatabase.show()
        taskMgr.doMethodLater(DatabaseGiveupTimeout, self._GuiScreen__giveUpWaitingForDatabase, 'waitingForDatabase')
        return Task.done

    
    def _GuiScreen__giveUpWaitingForDatabase(self, task):
