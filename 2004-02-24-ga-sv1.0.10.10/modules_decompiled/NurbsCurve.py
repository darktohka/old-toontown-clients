...
class NurbsCurve(PiecewiseCurve.PiecewiseCurve, NurbsCurveInterface.NurbsCurveInterface, FFIExternalObject.FFIExternalObject):
    ...
    def listTags(self, *_args):
        ...
        if numArgs == 1:
            import Ostream
            if isinstance(_args[0], Ostream.Ostream):
                return self._NurbsCurve__overloaded_listTags_ptrConstPandaNode_ptrOstream(_args[0])
            else:
                raise TypeError, 'Invalid argument 0, expected one of: <Ostream.Ostream> '
        elif numArgs == 2:
            import Ostream
            if isinstance(_args[0], Ostream.Ostream):
                if isinstance(_args[1], types.StringType):
                    return self._NurbsCurve__overloaded_listTags_ptrConstPandaNode_ptrOstream_atomicstring(_args[0], _args[1])
                else:
