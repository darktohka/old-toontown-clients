...
class LocalToon(DistributedToon.DistributedToon, LocalAvatar.LocalAvatar):
    ...
    def announceGenerate(self):
        ...
        self.book.addPage(self.optionsPage, pageName = Localizer.OptionsPageTitle)
        self.shardPage = ShardPage.ShardPage()
        self.shardPage.load()
        self.book.addPage(self.shardPage, pageName = Localizer.ShardPageTitle)
        self.mapPage = MapPage.MapPage()
        self.mapPage.load()
        self.book.addPage(self.mapPage, pageName = Localizer.MapPageTitle)
        self.invPage = InventoryPage.InventoryPage()
        self.invPage.load()
        self.book.addPage(self.invPage, pageName = Localizer.InventoryPageTitle)
        self.questPage = QuestPage.QuestPage()
        self.questPage.load()
        self.book.addPage(self.questPage, pageName = Localizer.QuestPageToonTasks)
        self.trackPage = TrackPage.TrackPage()
        self.trackPage.load()
        self.book.addPage(self.trackPage, pageName = Localizer.TrackPageShortTitle)
        self.suitPage = SuitPage.SuitPage()
        self.suitPage.load()
        self.book.addPage(self.suitPage, pageName = Localizer.SuitPageTitle)
        if base.config.GetBool('want-photo-album', 0):
            self.photoAlbumPage = PhotoAlbumPage.PhotoAlbumPage()
            self.photoAlbumPage.load()
            self.book.addPage(self.photoAlbumPage, pageName = Localizer.PhotoPageTitle)
        
        self.fishPage = FishPage.FishPage()
        self.fishPage.setAvatar(self)
        self.fishPage.load()
        self.book.addPage(self.fishPage, pageName = Localizer.FishPageTitle)
    ...
    def loadDisguisePages(self):
        ...
        self.book.addPage(self.disguisePage, pageName = Localizer.DisguisePageTitle)
        self.sosPage = NPCFriendPage.NPCFriendPage()
        self.sosPage.load()
        self.book.addPage(self.sosPage, pageName = Localizer.NPCFriendPageTitle)
    ...
    def _LocalToon__beginTossPie(self, time):
        ...
        if getattr(self.controlManager.walkControls, 'isAirborne', 0):
            return None
