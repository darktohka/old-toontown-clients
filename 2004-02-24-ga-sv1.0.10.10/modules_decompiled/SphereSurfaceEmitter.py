...
class SphereSurfaceEmitter(BaseParticleEmitter.BaseParticleEmitter, FFIExternalObject.FFIExternalObject):
    ...
    def constructor(self, *_args):
        numArgs = len(_args)
        if numArgs == 0:
            return self._SphereSurfaceEmitter__overloaded_constructor()
        elif numArgs == 1:
            if isinstance(_args[0], SphereSurfaceEmitter):
                return self._SphereSurfaceEmitter__overloaded_constructor_ptrConstSphereSurfaceEmitter(_args[0])
            else:
