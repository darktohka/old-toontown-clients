...
class ChatManager(PandaObject.PandaObject):
    ...
    def enterMainMenu(self):
        ...
        if toonbase.localToon.canChat() or toonbase.tcr.wantMagicWords:
    ...
    def enterUnpaidChatWarning(self):
        if self.paidNoParentPassword:
            directFrameText = Localizer.PaidNoParentPasswordWarning
            payButtonText = Localizer.PaidNoParentPasswordWarningSet
            directButtonText = Localizer.PaidNoParentPasswordWarningContinue
        else:
            directFrameText = Localizer.UnpaidChatWarning
            payButtonText = Localizer.UnpaidChatWarningPay
            directButtonText = Localizer.UnpaidChatWarningContinue
        if self.unpaidChatWarning == None:
            ...
            self.unpaidChatWarning = DirectFrame(pos = (0.0, 0.10000000000000001, 0.40000000000000002), relief = None, image = getDefaultDialogGeom(), image_color = ToontownGlobals.GlobalDialogColor, image_scale = (1.2, 1.0, 0.69999999999999996), text = directFrameText, text_wordwrap = 18, text_scale = 0.059999999999999998, text_pos = (0.0, 0.23000000000000001), textMayChange = 0)
            self.payButton = DirectButton(self.unpaidChatWarning, image = buttonImage, relief = None, text = payButtonText, image_scale = (1.75, 1, 1.1499999999999999), text_scale = 0.059999999999999998, text_pos = (0, -0.02), textMayChange = 0, pos = (0.0, 0.0, -0.080000000000000002), command = self._ChatManager__handleUnpaidChatWarningPay)
            DirectButton(self.unpaidChatWarning, image = buttonImage, relief = None, text = directButtonText, textMayChange = 0, image_scale = (1.75, 1, 1.1499999999999999), text_scale = 0.059999999999999998, text_pos = (0, -0.02), pos = (0.0, 0.0, -0.23000000000000001), command = self._ChatManager__handleUnpaidChatWarningContinue)
    ...
    def enterActivateChat(self):
        if self.activateChat == None:
            ...
            if toonbase.tcr.productName != 'NTT-DMC':
    ...
    def _ChatManager__normalButtonPressed(self):
        if toonbase.tcr.productName == 'DisneyOnline-US':
        ...
        elif toonbase.tcr.productName == 'Terra-DMC':
            if not toonbase.tcr.allowSecretChat():
                ...
                self.fsm.request('normalChat')
        elif toonbase.tcr.productName == 'NTT-DMC':
            if not toonbase.tcr.allowSecretChat():
                self.fsm.request('noSecretChatWarning')
            elif not toonbase.localToon.canChat():
                self.fsm.request('openChatWarning')
            ...
            print 'ChatManager: productName: %s not recognized' % toonbase.tcr.productName
