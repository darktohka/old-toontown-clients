...
class NameShop(PandaObject.PandaObject, StateData.StateData):
    ...
    def enter(self, toon, usedNames, warp):
        ...
        if not (self.addedGenderSpecific) or self.oldBoy != self.boy:
            ...
            self.titleCheck = self.makeCheckBox((-0.61699999999999999, 0, 0.374), Localizer.TitleCheckBox, (0, 0.25, 0.5, 1), self.titleToggle)
            self.firstCheck = self.makeCheckBox((-0.22800000000000001, 0, 0.374), Localizer.FirstCheckBox, (0, 0.25, 0.5, 1), self.firstToggle)
            self.lastCheck = self.makeCheckBox((0.38200000000000001, 0, 0.374), Localizer.LastCheckBox, (0, 0.25, 0.5, 1), self.lastToggle)
            del self.circle
        ...
        self.typeANameButton['text'] = Localizer.TypeANameButton
        if self.isLoaded == 0:
            self.load()
    ...
    def _NameShop__overflowNameInput(self):
        self.rejectName(Localizer.NameTooLong)
    ...
    def firstToggle(self, value):
        ...
        if self.chastise == 2:
            messenger.send('NameShop-mickeyChange', [
                [
                    Localizer.ApprovalForName1,
                    Localizer.ApprovalForName2]])
            self.chastise = 0
        ...
        if not (self.lastActive):
            ...
            messenger.send('NameShop-mickeyChange', [
                [
                    Localizer.MustHaveAFirstOrLast1,
                    Localizer.MustHaveAFirstOrLast2]])
            self.chastise = 1
    ...
    def load(self):
        ...
        self.nameLabel = OnscreenText.OnscreenText(Localizer.PleaseTypeName, parent = aspect2d, style = OnscreenText.ScreenPrompt, pos = (0, 0.53000000000000003))
        self.typeANameGUIElements.append(self.nameLabel)
        self.typeNotification = OnscreenText.OnscreenText(Localizer.AllNewNames, parent = aspect2d, style = OnscreenText.ScreenPrompt, pos = (0, 0.14999999999999999))
        ...
        self.submitButton = DirectButton(parent = aspect2d, relief = None, image = (guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale = (1.2, 0, 1.1000000000000001), pos = (-0.01, 0, -0.25), text = Localizer.NameShopSubmitButton, text_scale = 0.059999999999999998, text_pos = (0, -0.02), command = self._NameShop__typedAName)
        self.typeANameGUIElements.append(self.submitButton)
        self.randomButton = DirectButton(parent = aspect2d, relief = None, image = (guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale = (1.1499999999999999, 1.1000000000000001, 1.1000000000000001), scale = (1.3999999999999999, 1, 1.3999999999999999), pos = (0.01, 0, -0.5), text = Localizer.RandomButton, text_scale = 0.059999999999999998, text_pos = (0, -0.02), command = self._NameShop__randomName)
        self.pickANameGUIElements.append(self.randomButton)
        self.typeANameButton = DirectButton(parent = aspect2d, relief = None, image = (guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale = (1, 1.1000000000000001, 0.90000000000000002), pos = (0.01, 0, -0.69499999999999995), scale = (1.6000000000000001, 1, 1.7), text = Localizer.TypeANameButton, text_scale = 0.059999999999999998, text_pos = (0, -0.02), command = self._NameShop__typeAName)
        ...
        buttonImage = [
            ...
            imageList]
        buttonText = [
            Localizer.NameShopPay,
            Localizer.NameShopPlay]
        self.payDialog = DirectDialog(dialogName = 'paystate', topPad = 0, fadeScreen = 0.20000000000000001, pos = (0, 0.10000000000000001, 0.10000000000000001), button_relief = None, text_align = TextNode.ACenter, text = Localizer.NameShopOnlyPaid, buttonTextList = buttonText, buttonImageList = buttonImage, image_color = GlobalDialogColor, buttonValueList = [
            1,
        ...
        self.payDialog.hide()
        buttonText = [
            Localizer.NameShopContinueSubmission,
            Localizer.NameShopChooseAnother]
        self.approvalDialog = DirectDialog(dialogName = 'approvalstate', topPad = 0, fadeScreen = 0.20000000000000001, pos = (0, 0.10000000000000001, 0.10000000000000001), button_relief = None, image_color = GlobalDialogColor, text_align = TextNode.ACenter, text = Localizer.NameShopToonCouncil, buttonTextList = buttonText, buttonImageList = buttonImage, buttonValueList = [
            1,
    ...
    def _NameShop__createAvatar(self, *args):
        ...
        if not (self.avExists):
            ...
            self.rejectName(Localizer.EmptyNameError)
    ...
    def rejectName(self, str):
        ...
        self.names[0] = ''
        self.rejectDialog = ToontownDialog.GlobalDialog(doneEvent = 'rejectDone', message = str, style = ToontownDialog.Acknowledge)
    ...
    def findTempName(self):
        colorstring = Localizer.NumToColor[self.toon.style.headColor]
        animaltype = Localizer.AnimalToSpecies[self.toon.style.getAnimal()]
    ...
    def enterPayState(self):
        ...
        if toonbase.tcr.allowFreeNames() or self.isPaid:
            self.fsm.request('PickAName')
        else:
            tempname = self.findTempName()
            self.payDialog['text'] = Localizer.NameShopOnlyPaid + tempname
            self.payDialog.show()
    ...
    def _NameShop__typeAName(self):
        if self.fsm.getCurrentState().getName() == 'TypeAName':
            ...
            self.typeANameButton['text'] = Localizer.PickANameButton
    ...
    def enterApprovalState(self):
        ...
        tempname = self.findTempName()
        self.approvalDialog['text'] = Localizer.NameShopToonCouncil + tempname
    ...
    def enterAcceptedState(self):
        ...
        self.acceptedDialog = ToontownDialog.GlobalDialog(doneEvent = 'acceptedDone', message = Localizer.NameShopNameAccepted, style = ToontownDialog.Acknowledge)
    ...
    def enterRejectedState(self):
        ...
        self.rejectedDialog = ToontownDialog.GlobalDialog(doneEvent = 'rejectedDone', message = Localizer.NameShopNameRejected, style = ToontownDialog.Acknowledge)
    ...
    def handleSetNamePatternAnswerMsg(self, di):
        ...
        if newavId != self.avId:
            ...
            self.rejectName(Localizer.NameError)
    ...
    def serverCreateAvatar(self):
        ...
        style = self.toon.getStyle()
        avDNA = style.makeNetString()
        avNameIndices = self.nameIndices
        avNameFlags = self.nameFlags
        datagram = Datagram()
        datagram.addUint16(CLIENT_CREATE_AVATAR)
        datagram.addUint16(0)
        datagram.addString(avDNA)
        datagram.addUint8(self.index)
        self.newDNA = avDNA
        messenger.send('nameShopPost', [
            datagram])
        return None
    ...
    def waitForServer(self):
        self.waitForServerDialog = ToontownDialog.ToontownDialog(text = Localizer.WaitingForNameSubmission, style = ToontownDialog.NoButtons)
