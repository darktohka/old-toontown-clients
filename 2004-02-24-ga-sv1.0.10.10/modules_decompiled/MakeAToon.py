...
class MakeAToon(PandaObject.PandaObject, StateData.StateData):
    ...
    def genderShopOpening(self, task):
        self.host = self.minnie
        self.hostSez([
            Localizer.GenderShopQuestionMinnie], self.minnieDialogueArray[0])
        if base.config.GetString('language', 'english') == 'japanese':
    ...
    def genderShopOpening2(self, task = None):
        self.host = self.mickey
        self.hostSez([
            Localizer.GenderShopQuestionMickey], self.mickeyDialogueArray[0])
        return Task.done
    ...
    def enterGenderShop(self):
        ...
        self.minnie.setPosHpr(-2.7000000000000002, -1.8999999999999999, 0, 210, 0, 0)
        self.shop = GENDERSHOP
        self.guiTopBar['text'] = Localizer.CreateYourToonTitle
    ...
    def _MakeAToon__handleGenderShopDone(self):
        ...
        if self.toon.style.gender == 'm':
            ...
            gendershopInterval.append(Func(self.charSez, self.minnie, Localizer.GenderShopSeeYou, self.minnieDialogueArray[2]))
            self.host = self.mickey
            self.hostSez([
                Localizer.GenderShopFollow], self.mickeyDialogueArray[1])
        else:
            self.timeToGetOff = self.leftTime
            self.mickey.clearChat()
            gendershopInterval.append(Func(self.charSez, self.mickey, Localizer.GenderShopSeeYou, self.mickeyDialogueArray[2]))
            self.host = self.minnie
            self.hostSez([
                Localizer.GenderShopFollow], self.minnieDialogueArray[1])
    ...
    def bodyShopDialogTask(self, task):
        self.hostSez([
            Localizer.MakeAToonClickForNextScreen])
        return Task.done
    ...
    def enterColorShop(self):
        ...
        self.guiTopBar['text'] = Localizer.PaintYourToonTitle
    ...
    def colorShopDialogTask(self, task):
        self.hostSez([
            Localizer.MakeAToonYouCanGoBack])
        return Task.done
    ...
    def enterClothesShop(self):
        ...
        self.guiTopBar['text'] = Localizer.PickClothesTitle
        self.guiTopBar['text_fg'] = (1, 0.90000000000000002, 0, 1)
    ...
    def nameShopOpening(self, task):
        ...
        if NAMESHOP not in self.shopsVisited:
            ...
            self.hostSez([
                Localizer.MakeAFunnyName])
        else:
            self.hostSez([
                Localizer.MakeAToonLastStep,
                Localizer.PickANameYouLike])
        return Task.done
    ...
    def enterNameShop(self):
        self.shop = NAMESHOP
        self.roomWalls.setColorScale(0.90000000000000002, 0.75, 0.45000000000000001, 1)
        self.guiTopBar['text'] = Localizer.NameToonTitle
        self.guiTopBar['text_fg'] = (0.0, 0.65000000000000002, 0.34999999999999998, 1)
    ...
    def rejectName(self):
        self.ns.rejectName(Localizer.RejectNameText)
