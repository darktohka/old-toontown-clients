...
class Launcher(DirectObject):
    ...
    def downloadPatchProgress(self, task):
        bytesWritten = self.httpChannel.getBytesDownloaded()
    ...
    def downloadPatches(self):
        if len(self.patchList) > 0:
            ...
            if self.currentPhase > 3:
                self.download(serverPatchFilePath, localPatchFilename, self.downloadPatchDone, None)
            else:
                self.download(serverPatchFilePath, localPatchFilename, self.downloadPatchDone, self.downloadPatchProgress)
        else:
    ...
    def getParentPasswordSet(self):
        return self.getRegistry(self.chatterboxKey, 0)
