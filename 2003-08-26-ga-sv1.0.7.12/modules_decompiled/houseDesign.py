...
class ObjectManager(NodePath, PandaObject):
    ...
    def exitFurnitureMode(self, furnitureManager):
        ...
        toonbase.localToon.setGhostMode(0)
        toonbase.localToon.disableSlideMode()
        self.exitDeleteMode()
    ...
    def deleteItemFromRoom(self, dfitem, objectId):
        messenger.send('wakeup')
        callback = PythonUtil.Functor(self._ObjectManager__deleteItemFromRoomCallback, objectId)
        self.furnitureManager.deleteItemFromRoom(dfitem, callback)

    
    def _ObjectManager__deleteItemFromRoomCallback(self, objectId, retcode, item):
        if retcode < 0:
    ...
    def makeHouseExtentsBox(self):
        ...
        self.houseExtents = render.attachNewNode('furnitureCollisionNode')
        mx = targetBounds[0][0]
        Mx = targetBounds[1][0]
        my = targetBounds[0][1]
        My = targetBounds[1][1]
    ...
    def createVerifyDialog(self, item, verifyText, okFunc, cancelFunc):
        if self.verifyFrame == None:
            ...
            self.verifyFrame = DirectFrame(pos = (0.0, 0.10000000000000001, 0.10000000000000001), relief = None, image = getDefaultDialogGeom(), image_color = ToontownGlobals.GlobalDialogColor, image_scale = (1.2, 1, 1.3), text = '', text_wordwrap = 19, text_scale = 0.059999999999999998, text_pos = (0, 0.5), textMayChange = 1, sortOrder = NO_FADE_SORT_INDEX)
    ...
    def _ObjectManager__handleVerifyDeleteOK(self):
        (item, itemIndex, deleteFunction) = self.verifyItems
        self._ObjectManager__cleanupVerifyDelete()
        deleteFunction(item, itemIndex)
    ...
    def requestDelete(self, item, itemIndex, deleteFunction):
        ...
        self.createVerifyDialog(item, Localizer.HDDeleteItem, self._ObjectManager__handleVerifyDeleteOK, self._ObjectManager__cleanupVerifyDelete)
        self.verifyItems = (item, itemIndex, deleteFunction)

    
    def requestRoomDelete(self, dfitem, objectId):
        self._ObjectManager__cleanupVerifyDelete()
        item = dfitem.item
        if self.furnitureManager.ownerId != toonbase.localToon.doId or not item.isDeletable():
            self.warnNonDeletableItem(item)
        ...
        self.createVerifyDialog(item, Localizer.HDDeleteItem, self._ObjectManager__handleVerifyDeleteOK, self._ObjectManager__cleanupVerifyDelete)
        self.verifyItems = (dfitem, objectId, self.deleteItemFromRoom)
    ...
    def warnNonDeletableItem(self, item):
        ...
        self.nonDeletableItem = ToontownDialog.ToontownDialog(text = message, style = ToontownDialog.Acknowledge, fadeScreen = 0, command = self._ObjectManager__cleanupVerifyDelete)
    ...
    def requestReturnToAttic(self, item, objectId):
        ...
        if self.deleteMode:
            ...
            self.requestRoomDelete(dfitem, objectId)
            return None
        
        self.createVerifyDialog(item, Localizer.HDReturnVerify, self._ObjectManager__handleVerifyReturnOK, self._ObjectManager__cleanupVerifyDelete)
    ...
    def requestReturnToAtticFromTrash(self, item, itemIndex):
        self._ObjectManager__cleanupVerifyDelete()
        self.createVerifyDialog(item, Localizer.HDReturnFromTrashVerify, self._ObjectManager__handleVerifyReturnFromTrashOK, self._ObjectManager__cleanupVerifyDelete)
    ...
    def _ObjectManager__recoverDeletedItemCallback(self, retcode, item, itemIndex):
        ...
        if itemType == CatalogItemTypes.WALLPAPER_ITEM:
