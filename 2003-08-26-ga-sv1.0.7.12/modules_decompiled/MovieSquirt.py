    ...
    if hp > 0:
        ...
        if kbbonus > 0:
            ...
            if suitType == 'a':
                ...
                animTrack.append(ActorInterval(suit, 'slip-forward', startTime = 1.9399999999999999, duration = 1.03))
            elif suitType == 'c':
        ...
        if kbbonus > 0:
            ...
            bonusTrack.append(Func(suit.showLaffNumber, -kbbonus, 2, openEnded = 0))
        
        if hpbonus > 0:
            bonusTrack.append(Wait(0.75))
            bonusTrack.append(Func(suit.showLaffNumber, -hpbonus, 1, openEnded = 0))
...
def __doFireHose(squirt, delay, fShowStun):
    ...
    hands = toon.getRightHands()
    scale = ToontownGlobals.toonBodyScales[toon.style.getAnimal()]
