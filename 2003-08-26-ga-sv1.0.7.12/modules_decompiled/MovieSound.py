...
def doSounds(sounds):
    ...
    for soundList in prevSounds:
        if len(soundList) > 0:
            mtrack.append(__doSoundsLevel(soundList, delay, hitCount))
    ...
    for sound in sounds:
        ...
        for target in targets:
            ...
            if totalDamage > 0 and sound == lastSoundThatHit:
                ...
                suitTrack = Sequence()
                showDamage = Func(suit.showLaffNumber, -totalDamage, openEnded = 0)
                ...
                bonusTrack = None
                if hpbonus > 0:
                    bonusTrack = Sequence(Wait(delay + tSuitReact + 0.75), Func(suit.showLaffNumber, -hpbonus, 1, openEnded = 0))
