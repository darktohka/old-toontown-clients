...
class DistributedCloset(DistributedFurnitureItem.DistributedFurnitureItem):
    ...
    def setupCollisionSphere(self):
        if self.ownerId:
            ...
            self.closetSphereNode = CollisionNode(self.uniqueName('closetSphere'))
    ...
    def enterOpen(self):
        if self.ownerId:
            ...
            if self.av:
                ...
                self.av.stopSmooth()
                self.avMoveTrack = Sequence(Parallel(Func(self.av.play, 'walk'), LerpPosHprInterval(node = self.av, other = self, duration = 1.0, pos = Vec3(1.6699999999999999, -3.29, 0.025000000000000001), hpr = Vec3(112, 0, 0), blendType = 'easeOut')), Func(self.av.loop, 'neutral'), Func(self.av.startSmooth))
    ...
    def setCustomerDNA(self, avId, dnaString):
        if avId:
            ...
            if av:
                if self.av == toonbase.tcr.doId2do[avId]:
    ...
    def _DistributedCloset__popupAreYouSurePanel(self):
        ...
        DirectButton(self.popupInfo, image = okButtonImage, relief = None, text = Localizer.ClosetPopupOK, text_scale = 0.050000000000000003, text_pos = (0.0, -0.10000000000000001), textMayChange = 0, pos = (-0.10000000000000001, 0.0, -0.20999999999999999), command = self._DistributedCloset__handleYesImSure)
        DirectButton(self.popupInfo, image = cancelButtonImage, relief = None, text = Localizer.ClosetPopupCancel, text_scale = 0.050000000000000003, text_pos = (0.0, -0.10000000000000001), textMayChange = 0, pos = (0.10000000000000001, 0.0, -0.20999999999999999), command = self._DistributedCloset__handleNotSure)
        buttons.removeNode()
        self.popupInfo.reparentTo(aspect2d)
