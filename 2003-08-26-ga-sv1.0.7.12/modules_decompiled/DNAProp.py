...
class DNAProp(DNANode.DNANode, FFIExternalObject.FFIExternalObject):
    ...
    def constructor(self, *_args):
        numArgs = len(_args)
        if numArgs == 0:
            return self._DNAProp__overloaded_constructor()
        elif numArgs == 1:
            if isinstance(_args[0], types.StringType):
                return self._DNAProp__overloaded_constructor_atomicstring(_args[0])
            elif isinstance(_args[0], DNAProp):
                return self._DNAProp__overloaded_constructor_ptrConstDNAProp(_args[0])
            else:
