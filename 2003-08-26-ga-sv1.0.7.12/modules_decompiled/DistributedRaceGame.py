...
class DistributedRaceGame(DistributedMinigame):
    ...
    def getTitle(self):
        return Localizer.RaceGameTitle

    
    def getInstructions(self):
        return Localizer.RaceGameInstructions
    ...
    def enterMoveAvatars(self, choiceList, positionList, rewardList):
        self.notify.debug('enterMoveAvatars')
        tasks = []
        ...
        tasks.append(Task.Task(self.walkDone))
    ...
    def getLongestLerpTime(self, afterFirst):
        ...
        for i in range(len(self.choiceList)):
            ...
            if afterFirst or freq == 1:
                oldPosition = self.avatarPositions[self.avIdList[i]]
                squares_walked = self.positionList[i] - oldPosition
    ...
    def showChanceCard(self, task):
        ...
        task.chanceCard.lerpPosHpr(19.620000000000001, 13.41, 13.140000000000001, 355.24000000000001, 270, 90, 1.0, other = camera, task = 'cardLerp')
        return Task.done
    ...
    def showChanceCardText(self, task):
        ...
        cardText = ''
        if rewardEntry[1] != -1:
            rewardstr_fmt = Localizer.RaceGameCardText
            if rewardEntry[2] > 0:
                rewardstr_fmt = Localizer.RaceGameCardTextBeans
            
            cardText = rewardstr_fmt % {
                'name': name,
                'reward': rewardEntry[1] }
        else:
            rewardstr_fmt = Localizer.RaceGameCardTextHi1
            cardText = rewardstr_fmt % {
                'name': name }
    ...
    def exitMoveAvatars(self):
        ...
        for lane in range(self.numPlayers):
            ...
            taskMgr.remove('startNeutral-' + str(lane))
            taskMgr.remove('walkAvatar-' + str(lane))
            taskMgr.remove('runAvatar-' + str(lane))
