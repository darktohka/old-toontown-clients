...
class DistributedCannon(DistributedObject.DistributedObject):
    ...
    def __init__(self, cr):
        ...
        self.codeFSM = FSM.FSM('CannonCode', [
    ...
    def getParentNodePath(self):
        return toonbase.tcr.playGame.hood.loader.geom
    ...
    def setMovie(self, mode, avId):
        ...
        if mode == CannonGlobals.CANNON_MOVIE_CLEAR:
            ...
            if self.avId == toonbase.localToon.doId:
                toonbase.localToon.pose('lose', 110)
                toonbase.localToon.pose('slip-forward', 25)
    ...
    def setupMovingShadow(self):
        ...
        self.cRay = CollisionRay(0.0, 0.0, 20.0, 0.0, 0.0, -1.0)
    ...
    def _DistributedCannon__createToonModels(self):
        ...
        for dropShadow in self.av.dropShadows:
            dropShadow.hide()
    ...
    def _DistributedCannon__fireCannonTask(self, task):
        ...
        base.cTrav.addCollider(self.flyColNode, self.handler)
    ...
    def _DistributedCannon__stopCollisionHandler(self, avatar):
        if avatar:
            avatar.loop('neutral')
            if self.flyColNode:
                base.cTrav.removeCollider(self.flyColNode)
                self.flyColNode = None
            
            if avatar == toonbase.localToon:
                avatar.collisionsOn()
    ...
    def _DistributedCannon__handleCannonHit(self, collisionEntry):
        ...
        vel.normalize()
        space = collisionEntry.getIntoSpace()
        intoNormal = collisionEntry.getIntoSurfaceNormal()
    ...
    def _DistributedCannon__hitBumper(self, avatar, collisionEntry, sound, kr = 0.59999999999999998, angVel = 1):
        ...
        normal = collisionEntry.getIntoSurfaceNormal()
        normal.normalize()
        space = collisionEntry.getIntoSpace()
    ...
    def _DistributedCannon__hitRoof(self, avatar, collisionEntry, extraArgs = []):
        roof = collisionEntry.getIntoNodePath().getParent()
        normal = collisionEntry.getIntoSurfaceNormal()
        normal.normalize()
    ...
    def _DistributedCannon__flyTask(self, task):
        ...
        if self.hitBumper:
            ...
            view = 2
        deltaX = Vec3(pos - lastPos)
        relativeVec = self.av.getRelativeVector(render, deltaX)
        self.flyColNode.setVelocity(relativeVec)
        self.vel = relativeVec
    ...
    def _DistributedCannon__resetToonToCannon(self, avatar):
        ...
        if not avatar:
            if self.avId:
                avatar = toonbase.tcr.doId2do.get(self.avId, None)
