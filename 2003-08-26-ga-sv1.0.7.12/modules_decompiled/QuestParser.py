...
def init():
    globalVarDict.update({
        ...
        'chatScButton': toonbase.localToon.chatMgr.scButton,
...
class NPCMoviePlayer(DirectObject):
    ...
    def parseUnloadChar(self, line):
        ...
        track = Sequence()
        track.append(Func(char.removeActive))
        if char.style.name == 'mk' or char.style.name == 'mn':
            track.append(Func(char.stopEarTask))
        
        track.append(Func(char.delete))
    ...
    def parseLockLocalToon(self, line):
        return Sequence(Func(self.toon.detachCamera), Func(self.toon.disableAvatarControls), Func(self.toon.stopTrackAnimToSpeed), Func(self.toon.stopUpdateSmartCamera))

    
    def parseFreeLocalToon(self, line):
        return Sequence(Func(self.toon.attachCamera), Func(self.toon.startTrackAnimToSpeed), Func(self.toon.enableAvatarControls), Func(self.toon.startUpdateSmartCamera))
