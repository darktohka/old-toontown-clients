...
if base.config.GetBool('want-coghq', 0):
...
class MagicWordManager(DistributedObject.DistributedObject):
    ...
    def doMagicWord(self, word, avId, zoneId):
        ...
        elif base.config.GetBool('want-coghq', 0) and word == '~factory':
            if hasattr(toonbase.localToon, 'zoneB4factory'):
                toonbase.tcr.sendSetZoneMsg(toonbase.localToon.zoneB4factory)
                del toonbase.localToon.zoneB4factory
            else:
                gotZoneEvent = FactoryManager.FactoryManager.SetFactoryZoneMsg
                
                def handleFactoryZoneResponse(zoneId):
                    self.ignore(gotZoneEvent)
                    toonbase.tcr.sendSetZoneMsg(zoneId)

                self.accept(gotZoneEvent, handleFactoryZoneResponse)
                toonbase.tcr.factoryMgr.getFactoryZone(toonbase.localToon.doId)
                toonbase.localToon.zoneB4factory = toonbase.tcr.playGame.getPlace().getZoneId()
