...
def getToonTakeDamageTrack(toon, died, dmg, delay, damageAnimNames = None, splicedDamageAnims = None, showDamageExtraTime = 0.01):
    ...
    if died != 0:
        ...
        toonTrack.append(Parallel(loseIval, Sequence(Wait(delay), LerpScaleInterval(toon, shrinkDur, MovieUtil.PNT3_NEARZERO)), soundTrack))
        toonTrack.append(Func(toon.reparentTo, hidden))
        toonTrack.append(Func(toon.setScale, MovieUtil.PNT3_ONE))
    else:
        toonTrack.append(Func(toon.loop, 'neutral'))
...
def doPoundKey(attack):
    ...
    receiverPosPoints = [
        Point3(0.23000000000000001, 0.17000000000000001, -0.11),
        Point3(-6.0499999999999998, -2.5099999999999998, 177.58000000000001)]
    propTrack = Sequence(Wait(0.29999999999999999), Func(__showProp, phone, suit.getLeftHand(), phonePosPoints[0], phonePosPoints[1]), Func(__showProp, receiver, suit.getLeftHand(), receiverPosPoints[0], receiverPosPoints[1]), LerpScaleInterval(phone, 0.5, MovieUtil.PNT3_ONE, MovieUtil.PNT3_NEARZERO), Wait(0.73999999999999999), Func(receiver.wrtReparentTo, suit.getRightHand()), LerpPosHprInterval(receiver, 0.0001, Point3(-0.45000000000000001, 0.47999999999999998, -0.62), Point3(-82.569999999999993, 71.109999999999999, -89.480000000000004)), Wait(3.1400000000000001), Func(receiver.wrtReparentTo, phone), Wait(0.62), LerpScaleInterval(phone, 0.5, MovieUtil.PNT3_NEARZERO), Func(MovieUtil.removeProps, [
        receiver,
        phone]))
...
def doCanned(attack):
    ...
    if dmg > 0:
        ...
        hprTrack = Sequence(Wait(propDelay + suitDelay), LerpHprInterval(can, throwDuration, canHpr), Wait(0.40000000000000002), LerpHprInterval(can, 0.40000000000000002, Point3(-96.340000000000003, -181.62, -18.02)), LerpHprInterval(can, 0.29999999999999999, Point3(6.3399999999999999, -91.620000000000005, -18.02)), LerpHprInterval(can, 0.20000000000000001, Point3(96.340000000000003, 1.6200000000000001, -181.02000000000001)))
...
def doPinkSlip(attack):
    ...
    if dmg > 0:
        ...
        paperSpinTrack = Sequence(Wait(throwDelay), LerpHprInterval(paper, throwDuration, Point3(300, 0, 0)), Wait(paperPause), LerpHprInterval(paper, 1.3, Point3(-200, 100, 100)))
    else:
        ...
        paperSpinTrack = Sequence(Wait(throwDelay), LerpHprInterval(paper, throwDuration, Point3(300, 0, 0)), LerpHprInterval(paper, 0.5, Point3(10, 0, 0)))
    propTrack = Sequence()
...
def doReOrg(attack):
    ...
    if dmg > 0:
        ...
        for partNum in range(0, headParts.getNumPaths()):
            ...
            p = part.getP()
            r = part.getR()
            headTracks.append(Sequence(Wait(attackDelay), LerpPosInterval(part, 0.10000000000000001, Point3(x - 0.20000000000000001, y, z - 0.029999999999999999)), LerpPosInterval(part, 0.10000000000000001, Point3(x + 0.40000000000000002, y, z - 0.029999999999999999)), LerpPosInterval(part, 0.10000000000000001, Point3(x - 0.40000000000000002, y, z - 0.029999999999999999)), LerpPosInterval(part, 0.10000000000000001, Point3(x + 0.40000000000000002, y, z - 0.029999999999999999)), LerpPosInterval(part, 0.10000000000000001, Point3(x - 0.20000000000000001, y, z - 0.040000000000000001)), LerpPosInterval(part, 0.25, Point3(x, y, z + 2.2000000000000002)), LerpHprInterval(part, 0.40000000000000002, Point3(360, 0, 180)), LerpPosInterval(part, 0.29999999999999999, Point3(x, y, z + 3.1000000000000001)), LerpPosInterval(part, 0.14999999999999999, Point3(x, y, z + 0.29999999999999999)), Wait(0.14999999999999999), LerpHprInterval(part, 0.59999999999999998, Point3(-745, 0, 180), startHpr = Point3(0, 0, 180)), LerpHprInterval(part, 0.80000000000000004, Point3(25, 0, 180), startHpr = Point3(0, 0, 180)), LerpPosInterval(part, 0.14999999999999999, Point3(x, y, z + 1)), LerpHprInterval(part, 0.29999999999999999, Point3(h, p, r)), Wait(0.20000000000000001), LerpPosInterval(part, 0.10000000000000001, Point3(x, y, z)), Wait(0.90000000000000002)))
        
        
        def getChestTrack(part, attackDelay = attackDelay):
            origScale = part.getScale()
            return Sequence(Wait(attackDelay), LerpHprInterval(part, 1.1000000000000001, Point3(180, 0, 0)), Wait(1.1000000000000001), LerpHprInterval(part, 1.1000000000000001, part.getHpr()))
        ...
        if dmg > 0:
            ...
            def showShadows(true, shadows = shadows):
                ...
                for shadow in shadows:
                    if true == 0:
                        track.append(Func(shadow.hide))
                    else:
                        track.append(Func(shadow.show))
            ...
            shakeTrack.append(showShadows(0))
            ...
            shakeTrack.append(showShadows(1))
...
def doWatercooler(attack):
    ...
    toonTrack = getToonTrack(attack, suitTrack.getDuration() - 1.5, [
        ...
        'sidestep'])
    soundTrack = Sequence(Wait(1.1000000000000001), SoundInterval(globalBattleSoundCache.getSound('SA_watercooler_appear_only.mp3'), node = suit), Wait(0.40000000000000002), SoundInterval(globalBattleSoundCache.getSound('SA_watercooler_spray_only.mp3'), node = suit))
...
def doPeckingOrder(attack):
    ...
    for i in range(0, numBirds):
        ...
        birdTrack = Sequence(Wait(throwDelay), Func(battle.movie.needRestoreRenderProp, next), Func(next.wrtReparentTo, battle), Func(next.setHpr, Point3(90, -20, -40)), LerpPosInterval(next, 1.1000000000000001, hitPoint))
