...
class AvatarDetailPanel(DirectFrame):
    ...
    def enterQuery(self):
        ...
        self.bCancel.show()
        self.avatar = toonbase.tcr.doId2do.get(self.avId)
    ...
    def _AvatarDetailPanel__showData(self):
        ...
        if online:
            shardName = toonbase.tcr.getShardName(av.defaultShard)
            hoodName = toonbase.tcr.hoodMgr.getFullnameFromId(av.lastHood)
            if ZoneUtil.isWelcomeValley(av.lastHood):
                shardName = '%s (%s)' % (Localizer.WelcomeValley[1], shardName)
