...
class Loader:
    ...
    def loadFont(self, modelPath, priority = 0, faceIndex = 0, spaceAdvance = None, pointSize = None, pixelsPerUnit = None, scaleFactor = None, textureMargin = None, polyMargin = None, minFilter = None, magFilter = None, anisotropicDegree = None, lineHeight = None):
        if phaseChecker:
            phaseChecker(modelPath)
        
        fn = Filename(modelPath)
        extension = fn.getExtension()
        if extension == '' and extension == 'egg' or extension == 'bam':
            node = ModelPool.loadModel(modelPath)
            if node == None:
