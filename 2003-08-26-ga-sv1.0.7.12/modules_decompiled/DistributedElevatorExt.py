...
class DistributedElevatorExt(DistributedObject.DistributedObject):
    ...
    def setupNametag(self):
        if self.nametag == None:
            ...
            if name:
                name = Localizer.CogsIncModifier % name
            else:
    ...
    def setBldgDoId(self, bldgDoId):
        ...
        self.bldg = self.cr.doId2do.get(bldgDoId)
        if not (self.bldg):
            self.notify.error('setBldgDoId: elevator %d cannot find bldg %d!' % (self.doId, bldgDoId))
            return None
