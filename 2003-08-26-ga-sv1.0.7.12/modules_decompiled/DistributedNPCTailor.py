...
class DistributedNPCTailor(DistributedNPCToonBase):
        ...
        if mode == NPCToons.PURCHASE_MOVIE_TIMEOUT:
            ...
            if self.browsing == 0:
                if self.roomAvailable == 0:
                    self.setChatAbsolute(Localizer.STOREOWNER_NOROOM, CFSpeech | CFTimeout)
                else:
                    self.setChatAbsolute(Localizer.STOREOWNER_GREETING, CFSpeech | CFTimeout)
            else:
                self.setChatAbsolute(Localizer.STOREOWNER_BROWSING, CFSpeech | CFTimeout)
            if self.isLocalToon:
    ...
    def popupPurchaseGUI(self, task):
        ...
        if toonbase.localToon.getHeight() > 3.5:
            self.clothesGUI.topLButton.setZ(0.10000000000000001)
            self.clothesGUI.topRButton.setZ(0.10000000000000001)
            self.clothesGUI.bottomLButton.setZ(-0.29999999999999999)
            self.clothesGUI.bottomRButton.setZ(-0.29999999999999999)
        else:
            self.clothesGUI.topLButton.setZ(0)
            self.clothesGUI.topRButton.setZ(0)
            self.clothesGUI.bottomLButton.setZ(-0.40000000000000002)
            self.clothesGUI.bottomRButton.setZ(-0.40000000000000002)
    ...
    def _DistributedNPCTailor__handlePurchaseDone(self, timeout = 0):
        if self.clothesGUI.doneStatus == 'last' or timeout == 1:
            ...
            if self.roomAvailable == 0:
                if self.isLocalToon:
                    if self.av.isClosetFull() and which & ClosetGlobals.SHIRT and which & ClosetGlobals.SHORTS:
    ...
    def _DistributedNPCTailor__enterConfirmLoss(self, finished, which):
        if self.popupInfo == None:
            ...
            self.popupInfo = DirectFrame(parent = hidden, relief = None, state = 'normal', text = Localizer.STOREOWNER_CONFIRM_LOSS, text_wordwrap = 10, textMayChange = 0, frameSize = (-1, 1, -1, 1), text_pos = (0, -0.050000000000000003), geom = getDefaultDialogGeom(), geom_color = ToontownGlobals.GlobalDialogColor, geom_scale = (0.88, 1, 0.55000000000000004), geom_pos = (0, 0, -0.17999999999999999), text_scale = 0.080000000000000002)
            DirectButton(self.popupInfo, image = okButtonImage, relief = None, text = Localizer.STOREOWNER_OK, text_scale = 0.050000000000000003, text_pos = (0.0, -0.10000000000000001), textMayChange = 0, pos = (-0.080000000000000002, 0.0, -0.31), command = self._DistributedNPCTailor__handleConfirmLossOK, extraArgs = [
                finished,
                which])
            DirectButton(self.popupInfo, image = cancelButtonImage, relief = None, text = Localizer.STOREOWNER_CANCEL, text_scale = 0.050000000000000003, text_pos = (0.0, -0.10000000000000001), textMayChange = 0, pos = (0.080000000000000002, 0.0, -0.31), command = self._DistributedNPCTailor__handleConfirmLossCancel)
            buttons.removeNode()
    ...
    def setCustomerDNA(self, avId, dnaString):
        av = toonbase.tcr.doId2do.get(avId, None)
        if av:
            if self.av == av:
