...
class MakeAToon(PandaObject.PandaObject, StateData.StateData):
    ...
    def hostSez(self, stringList):
        self.host.setChatAbsolute(whrandom.choice(stringList), CFSpeech)
    ...
    def genderShopOpening(self, task):
        self.host = self.minnie
        self.hostSez([
            Localizer.GenderShopQuestionMinnie])
        self.host = self.mickey
        self.hostSez([
            Localizer.GenderShopQuestionMickey])
        return Task.done
    ...
    def _MakeAToon__handleGenderShopDone(self):
        ...
        if self.toon.style.gender == 'm':
            ...
            gendershopInterval.append(Func(self.charSez, self.minnie, Localizer.GenderShopSeeYou))
            self.host = self.mickey
            self.hostSez([
                Localizer.GenderShopFollow])
        else:
            self.timeToGetOff = self.leftTime
            self.mickey.clearChat()
            gendershopInterval.append(Func(self.charSez, self.mickey, Localizer.GenderShopSeeYou))
            self.host = self.minnie
            self.hostSez([
                Localizer.GenderShopFollow])
