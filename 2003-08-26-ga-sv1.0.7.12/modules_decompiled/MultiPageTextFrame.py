...
class MultiPageTextFrame(DirectFrame):
    ...
    def __init__(self, textList, width = defWidth, height = defHeight, wordWrap = None, hidePageNum = 0, pageChangeCallback = None, parent = aspect2d, **kw):
        ...
        self.nextButton = DirectButton(parent = self, relief = None, scale = buttonScale, image = (guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale = (0.75, 1, 1), pos = (hWidth * 0.34999999999999998, 0, buttonZ), text = Localizer.MultiPageTextFrameNext, text_scale = 0.050000000000000003, text_pos = (0, -0.02), command = self.turnPage, extraArgs = [
            1])
        self.prevButton = DirectButton(parent = self, relief = None, scale = buttonScale, image = (guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale = (0.75, 1, 1), pos = (-hWidth * 0.34999999999999998, 0, buttonZ), text = Localizer.MultiPageTextFramePrev, text_scale = 0.050000000000000003, text_pos = (0, -0.02), command = self.turnPage, extraArgs = [
            -1])
    ...
    def setPage(self, pageNum):
        ...
        if self.numPages == 1:
            ...
            self.nextButton.show()
            self.prevButton.show()
        self.pageNum['text'] = Localizer.MultiPageTextFramePage % (self.curPage + 1, self.numPages)
