...
class Launcher(DirectObject):
    ...
    def __init__(self):
        ...
        configGameServer = launcherConfig.GetString('game-server', '')
        if configGameServer:
            self.gameServer = URLSpec(configGameServer, 1)
            self.notify.info('Overriding gameServer to %s.' % self.gameServer.cStr())
        else:
            self.gameServer = gameServer
        configAccountServer = launcherConfig.GetString('account-server', '')
        if configAccountServer:
            self.accountServer = URLSpec(configAccountServer, 1)
            self.notify.info('Overriding accountServer to %s.' % self.accountServer.cStr())
        else:
        ...
        if proxies == '':
            ...
            self.notify.info('Download proxy: %s' % self.http.getProxySpec())
