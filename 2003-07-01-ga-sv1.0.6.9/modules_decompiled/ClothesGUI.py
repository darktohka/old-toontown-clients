...
class ClothesGUI(PandaObject.PandaObject, StateData.StateData):
    ...
    def load(self):
        ...
        self.topLButton = DirectButton(relief = None, image = (guiRArrowUp, guiRArrowDown, guiRArrowRollover, guiRArrowUp), image_scale = (-1, 1, 1), image3_color = Vec4(0.5, 0.5, 0.5, 0.75), text = Localizer.ClothesShopShirt, text_scale = 0.0625, text_pos = (0.025000000000000001, 0), text_fg = (0.80000000000000004, 0.10000000000000001, 0, 1), pos = topLPos, command = self.swapTop, extraArgs = [
            -1])
        self.topRButton = DirectButton(relief = None, image = (guiRArrowUp, guiRArrowDown, guiRArrowRollover, guiRArrowUp), image3_color = Vec4(0.5, 0.5, 0.5, 0.75), text = Localizer.ClothesShopShirt, text_scale = 0.0625, text_pos = (-0.025000000000000001, 0), text_fg = (0.80000000000000004, 0.10000000000000001, 0, 1), pos = topRPos, command = self.swapTop, extraArgs = [
            1])
    ...
    def setupButtons(self):
        ...
        if self.gender == 'f':
            ...
            if AvatarDNA.GirlBottoms[botTex][1] == AvatarDNA.SHORTS:
                self.girlInShorts = 1
            
        
        if self.toon.style.getGender() == 'm':
            self.bottomLButton['text'] = Localizer.ClothesShopShorts
            self.bottomRButton['text'] = Localizer.ClothesShopShorts
        else:
            self.bottomLButton['text'] = Localizer.ClothesShopBottoms
            self.bottomRButton['text'] = Localizer.ClothesShopBottoms
