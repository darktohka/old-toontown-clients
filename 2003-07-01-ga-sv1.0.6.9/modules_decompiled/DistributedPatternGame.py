...
class DistributedPatternGame(DistributedMinigame):
    ...
    strWatch = Localizer.PatternGameWatch
    strGo = Localizer.PatternGameGo
    strRight = Localizer.PatternGameRight
    strWrong = Localizer.PatternGameWrong
    strPerfect = Localizer.PatternGamePerfect
    strBye = Localizer.PatternGameBye
    strWaitingOtherPlayers = Localizer.PatternGameWaitingOtherPlayers
    strPleaseWait = Localizer.PatternGamePleaseWait
    minnieAnimNames = [
        'up',
        'left',
        'down',
    ...
    def getTitle(self):
        return Localizer.PatternGameTitle

    
    def getInstructions(self):
        return Localizer.PatternGameInstructions
    ...
    def _DistributedPatternGame__getDanceStepAnimTrack(self, toon, anim, speedScale):
        ...
        return Sequence(Func(toon.pingpong, anim, 0, numFrames - 1), Wait(self.getDanceStepDuration()))
    ...
    def changeArrow(self, toonID, index):
        self.arrowDict[toonID][0].setR(90 * index - 90)
    ...
    def getDanceSequenceButtonSoundTrack(self, pattern):
        tracks = []
        for buttonIndex in pattern:
            tracks.append(self.getDanceStepButtonSoundTrack(buttonIndex))
    ...
    def enterGetUserInput(self):
        ...
        self.setupTrack = Sequence(Func(self._DistributedPatternGame__setMinnieChat, self.strGo, 0), Func(self.setText, self.roundText, Localizer.PatternGameGo), Func(self.roundText.setScale, 0.29999999999999999), Func(enableKeys), Func(startTimer), Wait(0.80000000000000004), Func(self._DistributedPatternGame__clearMinnieChat), Func(self.setText, self.roundText, ' '), Func(self.roundText.setScale, 0.12), Func(self.setTextFG, self.roundText, self.normalTextColor))
    ...
    def enterPlayBackPatterns(self):
        ...
        if self.fastestAvId == self.localAvId:
            if self.numPlayers != 2:
                self.roundText['text'] = Localizer.PatternGameFastest
            else:
                self.roundText['text'] = Localizer.PatternGameFaster
            ...
            if self.round == PatternGameGlobals.NUM_ROUNDS:
                ...
                self.roundText['text'] = Localizer.PatternGameYouCanDoIt
            jumpTrack = Sequence(Wait(0.5), Wait(0.5))
        elif self.fastestAvId == 1:
            self.roundText['text'] = Localizer.PatternGameGreatJob
            jumpTrack = Sequence(Wait(0.5), Wait(0.5))
        else:
            ...
            if self.numPlayers != 2:
                rewardStr = Localizer.PatternGameOtherFastest
            else:
                rewardStr = Localizer.PatternGameOtherFaster
