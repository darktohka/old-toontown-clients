...
class QuestChoiceGui(DirectFrame):
    ...
    def __init__(self):
        ...
        self.cancelButton = DirectButton(parent = self, relief = None, image = (guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale = (0.69999999999999996, 1, 1), text = Localizer.QuestChoiceGuiCancel, text_scale = 0.059999999999999998, text_pos = (0, -0.02), command = self.chooseQuest, extraArgs = [
            0])
        guiButton.removeNode()
        self.timer = ToontownTimer.ToontownTimer()
        self.timer.reparentTo(self)
        self.timer.setScale(0.29999999999999999)
        toonbase.setCellsAvailable(toonbase.leftCells, 0)
        toonbase.setCellsAvailable([
            toonbase.bottomCells[0],
            toonbase.bottomCells[1]], 0)
    ...
    def chooseQuest(self, questId):
        toonbase.setCellsAvailable(toonbase.leftCells, 1)
        toonbase.setCellsAvailable([
            toonbase.bottomCells[0],
            toonbase.bottomCells[1]], 1)
        self.timer.stop()
