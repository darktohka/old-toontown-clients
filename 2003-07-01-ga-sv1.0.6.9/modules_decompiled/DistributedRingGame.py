...
class DistributedRingGame(DistributedMinigame):
    ...
    def setGameReady(self):
        ...
        if not self.isSinglePlayer():
            localToon = self.getAvatar(self.localAvId)
            self.pusher = CollisionHandlerPusher()
            self.pusher.addColliderNode(localToon.cSphereNode, localToon.node())
            localToon.cTrav.addCollider(localToon.cSphereNode, self.pusher)
    ...
    def enterCleanup(self):
        ...
        if not self.isSinglePlayer():
            localToon = self.getAvatar(self.localAvId)
            localToon.cTrav.removeCollider(localToon.cSphereNode)
            del self.pusher
