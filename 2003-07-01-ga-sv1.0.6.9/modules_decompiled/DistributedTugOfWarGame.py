...
class DistributedTugOfWarGame(DistributedMinigame):
    ...
    def sendStopSignal(self, winners, losers, tieers):
        ...
        if self.suit:
            if self.suitId in winners:
                ...
                if randInt < 3:
                    ...
                    reactIntervals.append(Sequence(Func(self.suit.loop, 'victory'), Wait(2.6000000000000001), LerpPosInterval(self.suit, duration = 2, pos = newPos), oopsTrack, Func(self.suit.loop, 'neutral')))
        ...
        for avId in self.avIdList:
            ...
            toon.loop('neutral')
            if avId in winners:
                reactIntervals.append(Func(toon.loop, 'victory'))
            elif avId in losers:
                reactIntervals.append(Func(toon.loop, 'neutral'))
            else:
                reactIntervals.append(Func(toon.loop, 'neutral'))
        
        if self.localAvId in winners:
            exitIntervals.append(Func(self.setText, self.roundText, Localizer.TugOfWarGameEnd))
            exitIntervals.append(Wait(5.0))
        elif self.localAvId in losers:
            exitIntervals.append(Func(self.setText, self.roundText, Localizer.TugOfWarGameEnd))
            exitIntervals.append(Wait(4.7999999999999998 + suitSlipTime))
        else:
            exitIntervals.append(Func(self.setText, self.roundText, Localizer.TugOfWarGameTie))
            exitIntervals.append(Wait(2.5))
        exitIntervals.append(Func(self.gameOver))
        self.showTrack = Parallel(Sequence(reactIntervals), Sequence(exitIntervals))
