...
class LaffMeter(DirectFrame):
    ...
    def animatedEffect(self, delta):
        ...
        if delta > 0:
            Sequence(self.container.scaleInterval(0.20000000000000001, 1.333, blendType = 'easeOut'), self.container.scaleInterval(0.20000000000000001, 1, blendType = 'easeIn'), name = taskName, autoFinish = 1).play()
        else:
            Sequence(self.container.scaleInterval(0.20000000000000001, 0.66600000000000004, blendType = 'easeOut'), self.container.scaleInterval(0.20000000000000001, 1, blendType = 'easeIn'), name = taskName, autoFinish = 1).play()

    
    def adjustFace(self, hp, maxHp, animate = 1):
    ...
    def start(self):
        ...
        if self.isToon:
            ...
            self.adjustFace(self.hp, self.maxHp, 0)
            if self.av:
