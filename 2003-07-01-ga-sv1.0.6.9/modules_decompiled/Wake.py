...
class Wake(NodePath):
    ...
    def createRipple(self, zPos, rate = 1.0, startFrame = 0):
        ...
        def clearDoLaterList(state):
            self.doLaters[state.rippleCount] = None
            return Task.done

        
        def destroyRipple(state):
            ripple.removeNode()
            return Task.done

        t = Task.Task(clearDoLaterList)
        t.rippleCount = self.rippleCount
        t.uponDeath = destroyRipple
        taskMgr.doLater(duration, t, 'wake-%d-destroy-%d' % (self.trackId, self.rippleCount))
